...
(PDF)» в `client/src/pages/AssemblePage.tsx`). Префикс `TC-P7-*` — Этап 7 (реальная загрузка/
постоянное хранение `chart-image`, `server/prisma/schema.prisma` модель `ChartImageAsset`,
`server/src/utils/chartImageStorage.ts`, `server/src/routes/slides.ts` (`POST`/`DELETE
/:id/blocks/:templateBlockId/chart-image`, гард no-op-only для `CHART_IMAGE` в общем `PATCH`),
`server/src/routes/uploads.ts` (`GET /api/uploads/chart-images/:assetId`), `server/src/pdf/
browser.ts` (фикс гонки `networkidle`), `client/src/components/slideBlocks.tsx`
(`ChartImageEditor`), `client/src/pages/SlideFormPage.tsx`, `client/src/pages/TemplatesPage.tsx`,
`client/src/pages/ReviewPage.tsx`). Префикс `TC-P8-*` — Этап 8 (внутриприложенческие уведомления,
`WeeklyCycle.deadline`, модель `Notification`, `server/src/utils/notifications.ts`,
`server/src/routes/notifications.ts`, `server/src/routes/weeklyCycles.ts` (`PATCH /:id` + `deadline`),
`server/src/routes/presentations.ts` (`getOrCreatePresentation` — триггер `CYCLE_ASSEMBLED`),
`client/src/components/NotificationBell.tsx`, `client/src/pages/CyclesPage.tsx`
(инлайн-редактирование строки цикла), deep-link `?cycle=`/`?template=` в
`client/src/pages/AssemblePage.tsx`/`SlideFormPage.tsx`/`ReviewPage.tsx`). Префикс `TC-P9-*` — Этап 9
(симметричная разборка презентации `ASSEMBLED → COLLECTING`, `POST /api/presentations/cycle/
:weeklyCycleId/disassemble` в `server/src/routes/presentations.ts`, аддитивное значение
`CYCLE_DISASSEMBLED` в `enum NotificationType`, кнопка «Разобрать» в `client/src/pages/
CyclesPage.tsx`, `case "CYCLE_DISASSEMBLED"` в `client/src/components/NotificationBell.tsx`).
Категории: `positive` / `negative` / `unit` / `integration` / `e2e`.
...
### `GET /api/uploads/chart-images/:assetId` — безопасность пути/раздача файла (Этап 7)

| ID | Название | Категория | Предусловия | Шаги | Ожидаемый результат |
|---|---|---|---|---|---|
| TC-P7-01 | Directory traversal / мусорные значения `assetId` (`../../../etc/passwd`, урл-кодированные варианты, `\`-путь Windows, SQL-инъекция в виде строки, не-UUID, `nil`-UUID, `%00`, строка на 5000 символов, `.`/`..`) | negative/integration | ADMIN | `GET /uploads/chart-images/<garbage>` × 10 вариантов, включая двойное URL-кодирование `..%2f` на HTTP-уровне (в обход `encodeURIComponent` на клиенте) | Каждый — чистый `404 {"error": "Файл не найден"}`, ни разу не 500/не зависание; `GET /api/health` быстро отвечает сразу после серии — сервер не пострадал. Механизм защиты подтверждён по коду: `assetId` используется только как Prisma PK для `findUnique`, не попадает в `path.join` до факта нахождения записи в БД |
| TC-P7-02 | Валидный, но несуществующий (nil) UUID | negative/integration | ADMIN | `GET /uploads/chart-images/00000000-0000-0000-0000-000000000000` | 404, тот же путь кода, что и TC-P7-01 |
| TC-P7-03 | Content-Type раздаваемого файла определяется словарём по расширению, а не эхом клиента | positive/integration | Загружены и PNG-, и JPEG-файлы (разные блоки) | `GET` каждого | `Content-Type: image/png` для PNG-ассета, `image/jpeg` для JPEG-ассета |

### `POST`/`DELETE /api/slides/:id/blocks/:templateBlockId/chart-image` — изоляция блоков и правильность привязки (Этап 7)

| ID | Название | Категория | Предусловия | Шаги | Ожидаемый результат |
|---|---|---|---|---|---|
| TC-P7-04 | `templateBlockId` из ДРУГОГО шаблона (не принадлежит шаблону этого слайда) | negative/integration | Слайд по шаблону A, отдельный шаблон B с собственным `CHART_IMAGE`-блоком | SPEAKER `POST /slides/:id/blocks/<blockId-шаблона-B>/chart-image` (multipart, валидный PNG) | 400 «Блок не найден или не является изображением», не 500 и не тихая запись не туда |
| TC-P7-05 | То же для `DELETE` | negative/integration | То же | SPEAKER `DELETE /slides/:id/blocks/<blockId-шаблона-B>/chart-image` | 400 |
| TC-P7-06 | `templateBlockId` существует в шаблоне ЭТОГО слайда, но блок НЕ `CHART_IMAGE` (напр. `RICH_TEXT_SECTION`) | negative/integration | Слайд с блоком `RICH_TEXT_SECTION` | SPEAKER `POST .../blocks/<rich-text-blockId>/chart-image` | 400 |
| TC-P7-07 | Полностью произвольная строка вместо `templateBlockId` | negative/integration | — | SPEAKER `POST .../blocks/not-a-real-id/chart-image` | 400, не 500 |
| TC-P7-08 | Два блока `CHART_IMAGE` на одном слайде — загрузка в один не затрагивает другой | positive/integration | Шаблон с 2 `CHART_IMAGE`-блоками, слайд создан | Загрузить PNG в блок 1, JPEG в блок 2 → прочитать оба ассета | Разные `asset.id`, оба читаются, правильные `Content-Type` каждый |
| TC-P7-09 | Замена картинки в блоке 2 не трогает файл/значение блока 1 | positive/integration | То же, после TC-P7-08 | Заменить картинку блока 2 → повторно прочитать ассет блока 1 (старый id) | Ассет блока 1 по-прежнему 200 (тот же id, не пересоздан); старый ассет блока 2 → 404 (удалён при замене), новый ассет блока 2 → 200 |
| TC-P7-10 | Удаление картинки в блоке 1 не трогает блок 2 | positive/integration | То же | `DELETE` блок 1 → `{path: null}` → прочитать текущий ассет блока 2 | Ассет блока 2 всё ещё 200; старый ассет блока 1 → 404 |

### `PATCH /api/slides/:id` — взаимодействие общего сохранения с загрузкой картинки (Этап 7)

| ID | Название | Категория | Предусловия | Шаги | Ожидаемый результат |
|---|---|---|---|---|---|
| TC-P7-11 | Клик «Сохранить» сразу после успешной загрузки картинки, без прочих правок | positive/integration | Картинка только что загружена дедикейтед-роутом | SPEAKER `PATCH /slides/:id {blockValues: <полный набор, включая {path: <тот же assetId>} для картиночного блока, без изменений>}` | 200, без 400 — no-op-гард для `CHART_IMAGE` в общем `PATCH` пропускает байт-в-байт совпадающее значение |
| TC-P7-12 | То же самое, но одновременно с РЕАЛЬНЫМ изменением другого (текстового) блока того же слайда | positive/integration | То же | `PATCH {blockValues: [{картинка, тот же path}, {текстовый блок, новое значение}]}` | 200; текстовое изменение реально сохранилось (подтверждено повторным `GET`); `path` картиночного блока не изменился |
| TC-P7-13 | Попытка подставить произвольный `path` для `CHART_IMAGE` через общий `PATCH` (не через дедикейтед upload) | negative/integration | Слайд с уже загруженной картинкой | `PATCH {blockValues: [{картиночный блок, value: {path: "sneaky-injected-path"}}]}` | 400 «Изображение для этого блока меняется отдельным запросом…»; повторный `GET` подтверждает — реальный `path` блока НЕ изменился на инъецированную строку (нет частичной порчи данных при отклонённом запросе) |

### `GET /api/uploads/chart-images/:assetId` — доступ на чтение после архивации цикла (Этап 7)

| ID | Название | Категория | Предусловия | Шаги | Ожидаемый результат |
|---|---|---|---|---|---|
| TC-P7-14 | Не-владелец SPEAKER читает картинку `IN_PRESENTATION`-слайда ДО архивации (регрессия/база для сравнения) | positive/integration | Слайд с картинкой отправлен, добавлен в презентацию (цикл → `ASSEMBLED`) | Второй SPEAKER (не владелец) `GET /uploads/chart-images/:assetId` | 200 — правило «просмотр презентации открыт всем авторизованным для не-архивных циклов» распространяется и на картинки |
| TC-P7-15 | **Тот же не-владелец SPEAKER после реальной архивации цикла (`POST /weekly-cycles/:id/archive`)** | negative/integration | Цикл из TC-P7-14, заархивирован | Тот же SPEAKER `GET /uploads/chart-images/:assetId` | 403 «Нет доступа к файлу» — доступ пересчитывается по актуальному статусу цикла, не кэширован с момента `IN_PRESENTATION` |
| TC-P7-16 | Владелец слайда сохраняет доступ к своей картинке после архивации | positive/integration | То же | Владелец SPEAKER `GET /uploads/chart-images/:assetId` | 200 |
| TC-P7-17 | ADMIN сохраняет доступ к любой картинке после архивации | positive/integration | То же | ADMIN `GET /uploads/chart-images/:assetId` | 200 |
| TC-P7-18 | Мутация (upload/delete) заблокирована для обеих ролей после архивации (регрессия статусного гарда) | negative/integration | То же | SPEAKER и ADMIN по отдельности пробуют `POST .../chart-image` на слайде архивного цикла | 403 у обоих — `checkImageMutationAccess` учитывает `weeklyCycle.status !== "COLLECTING"` независимо от роли |

### ADMIN редактирует картинку на чужом слайде (Этап 7)

| ID | Название | Категория | Предусловия | Шаги | Ожидаемый результат |
|---|---|---|---|---|---|
| TC-P7-19 | ADMIN загружает картинку на `DRAFT`-слайде другого SPEAKER (API) | positive/integration | Слайд SPEAKER A, `DRAFT`, цикл `COLLECTING` | ADMIN `POST /slides/:id/blocks/:blockId/chart-image` | 200; владелец (SPEAKER A) при повторном `GET /slides` видит тот же `assetId` — первый в проекте путь, где ADMIN напрямую меняет содержимое чужого слайда |
| TC-P7-20 | Не-владелец SPEAKER (не ADMIN) пробует то же самое | negative/integration | То же | SPEAKER B `POST /slides/:id/blocks/:blockId/chart-image` на слайде SPEAKER A | 404 «Слайд не найден» (существование чужого слайда не раскрывается, как и у остальных SPEAKER-роутов) |
| TC-P7-21 | ADMIN на `/review` видит `ChartImageEditor` (реальный `<input type="file">`), не read-only превью, для `CHART_IMAGE`-блока `DRAFT`/`NEEDS_REVISION`-слайда в `COLLECTING`-цикле | positive/e2e | Слайд SPEAKER, `DRAFT`, ещё не отправлен | ADMIN → `/review` → выбрать цикл | Карточка слайда со статусом «Черновик» видна (в отличие от старого поведения, где `/review` показывал только `SUBMITTED` для действий), внутри — файловый инпут, а не текст-заглушка |
| TC-P7-22 | Загрузка через этот UI реально применяется и видна владельцу слайда | positive/e2e | То же | ADMIN выбирает файл через `<input type="file">` на `/review` | Превью появляется на экране ADMIN; повторный `GET /slides` от имени SPEAKER (API) показывает тот же `assetId`, что и загрузил ADMIN |

### Конструктор шаблонов — разблокировка `CHART_IMAGE` (Этап 7)

| ID | Название | Категория | Предусловия | Шаги | Ожидаемый результат |
|---|---|---|---|---|---|
| TC-P7-23 | Чекбокс «обязательное» для блока типа `CHART_IMAGE`, добавленного вручную (не через импорт), НЕ задизейблен | positive/e2e | ADMIN на `/templates`, «+ Новый шаблон» | Сменить тип единственного блока на «Изображение (график/диаграмма)» через `<select>` | Чекбокс «обязательное» кликабелен (`disabled === false`), успешно ставится в `checked` |
| TC-P7-24 | Шаблон с вручную добавленным обязательным `CHART_IMAGE`-блоком сохраняется без ошибки | positive/e2e | То же | «Сохранить шаблон» | Без баннера `.error-text`, шаблон появляется в списке слева |
| TC-P7-25 | `isBlockEmpty` для `CHART_IMAGE` на форме слайда — подсказка «— не заполнено» до загрузки | positive/e2e | Шаблон из TC-P7-24, SPEAKER открывает форму слайда | Выбрать цикл+шаблон, ничего не загружая | Подсказка «— не заполнено» видна рядом с обязательным пустым картиночным блоком |
| TC-P7-26 | Подсказка исчезает сразу после успешной загрузки | positive/e2e | То же | Загрузить PNG через `<input type="file">` | Превью появляется, подсказка «— не заполнено» пропадает без перезагрузки страницы |
| TC-P7-27 | Персистентность после перезагрузки страницы (с повторным выбором цикла/шаблона — селект не хранит состояние в URL, это ожидаемо, не баг) | positive/e2e | То же | `page.reload()` → заново выбрать тот же цикл+шаблон | Картинка на месте, подсказка «не заполнено» по-прежнему отсутствует |
| TC-P7-28 | Подсказка возвращается после удаления картинки | positive/e2e | То же | Клик «Удалить» | Превью исчезает, подсказка «— не заполнено» появляется снова |
| TC-P7-29 | Клик «Сохранить» сразу после загрузки картинки в реальном UI (не только API) — регрессия/дубль TC-P7-11 через браузер | positive/e2e | То же, картинка только что загружена | Клик «Сохранить» | Без баннера ошибки, появляется подпись «Сохранено» |

### Целостность файлов на диске (Этап 7)

| ID | Название | Категория | Предусловия | Шаги | Ожидаемый результат |
|---|---|---|---|---|---|
| TC-P7-30 | Отсутствие «осиротевших» файлов/записей после серии upload/replace/delete в рамках прогона | positive/integration | Несколько циклов замены/удаления картинок за прогон (TC-P7-08..10, TC-P7-19..22, TC-P7-25..29) | Сравнить количество файлов в `server/uploads/chart-images/` с количеством строк `ChartImageAsset` в БД после прогона | Числа совпадают (22 файла = 22 строки на момент этой проверки) — ни одного файла без записи в БД, ни одной записи без файла на диске |

### Unit / статическая проверка типов (Этап 7)

| ID | Название | Категория | Шаги | Ожидаемый результат |
|---|---|---|---|---|
| TC-P7-U01 | `tsc --noEmit` чистый после правок Этапа 7 | unit | `npx tsc --noEmit` в `server/` и в `client/` | Без ошибок |

### `PATCH /api/weekly-cycles/:id` — поле `deadline` (Этап 8)

| ID | Название | Категория | Предусловия | Шаги | Ожидаемый результат |
|---|---|---|---|---|---|
| TC-P8-01 | `deadline` (ISO datetime) сохраняется | positive/integration | QA-цикл | ADMIN `PATCH {deadline: "<iso>"}` | 200, `deadline` в ответе === отправленное значение |
| TC-P8-02 | Отсутствие ключа `deadline` в теле не трогает поле | positive/integration | Цикл с уже заданным `deadline` | ADMIN `PATCH {weekLabel}` (без ключа `deadline`) | 200, `deadline` не изменился |
| TC-P8-03 | `status` в теле по-прежнему 400 (регрессия Этапа 5а) | negative(regression)/integration | Любой цикл | ADMIN `PATCH {status: "ASSEMBLED"}` | 400 |
| TC-P8-04 | `deadline: null` очищает поле | positive/integration | Цикл с заданным `deadline` | ADMIN `PATCH {deadline: null}` | 200, `deadline === null` |
| TC-P8-UI-06/07 | Кнопка «Изменить» доступна и на `ARCHIVED`-цикле, PATCH дедлайна проходит без ошибки | positive(edge)/e2e | ARCHIVED цикл в списке | ADMIN открывает инлайн-правку архивного цикла, меняет дедлайн, «Сохранить» | Без `.error-text`, значение сохраняется — бэкенд не гейтит `PATCH` по статусу ни для одного поля |

### Персистентные уведомления `CYCLE_ASSEMBLED`/`CYCLE_ARCHIVED` (Этап 8)

| ID | Название | Категория | Предусловия | Шаги | Ожидаемый результат |
|---|---|---|---|---|---|
| TC-P8-12/13/14 | Владелец слайда, чей слайд НЕ добавлен в презентацию, всё равно получает `CYCLE_ASSEMBLED` | positive/integration | Цикл с 2 спикерами, один слайд добавлен в презентацию, слайды/владелец другого — нет | ADMIN добавляет первый слайд в презентацию (триггер перехода в `ASSEMBLED`) | Оба владельца получают `CYCLE_ASSEMBLED`, включая того, чей слайд не в презентации |
| TC-P8-DB-1 | Один спикер с ≥2 слайдами в одном цикле получает РОВНО одну строку `Notification` на тип события (не по одной на слайд) | positive/integration | Спикер с 2 слайдами (разные шаблоны) в одном цикле | Сборка + архивация цикла | В БД по `(recipientId, type)` — не более 1 строки на каждую комбинацию, при 2 слайдах у владельца |
| TC-P8-15 | `ALL_SUBMITTED` исчезает сразу после перехода цикла в `ASSEMBLED` (цикл больше не `COLLECTING`) | positive/integration | Цикл со всеми слайдами `SUBMITTED`, только что собранный | ADMIN `GET /notifications` сразу после сборки | `ALL_SUBMITTED` для этого цикла отсутствует |
| TC-P8-23 | `POST /:id/archive` создаёт `CYCLE_ARCHIVED` для всех владельцев слайдов цикла | positive/integration | Собранный цикл (`ASSEMBLED`) с презентацией | ADMIN `POST /:id/archive` | 200, все владельцы слайдов цикла получают `CYCLE_ARCHIVED` |
| TC-P8-32/33 | Сборка/архивация цикла БЕЗ единого слайда (только заглушка) не создаёт уведомлений и не падает | positive(edge)/integration | Пустой цикл (0 `Slide`) | ADMIN добавляет заглушку → архивирует | Оба запроса 200/201, ноль строк `Notification`, без 500 |

### Живые условия — `DEADLINE_APPROACHING` / `NEEDS_REVISION` / `ALL_SUBMITTED` (Этап 8)

| ID | Название | Категория | Предусловия | Шаги | Ожидаемый результат |
|---|---|---|---|---|---|
| TC-P8-06/07 | `DEADLINE_APPROACHING` — только для `DRAFT`/`NEEDS_REVISION`-слайдов, не для `SUBMITTED` | positive+negative/integration | Один спикер, 2 слайда в одном цикле (`deadline` ≤24ч): один `DRAFT`, один `SUBMITTED` | SPEAKER `GET /notifications` | Пункт есть для `DRAFT`-слайда, отсутствует для `SUBMITTED` |
| TC-P8-08/10 | `ALL_SUBMITTED` — появляется только когда ВСЕ существующие слайды цикла `SUBMITTED` (≥1), пропадает при наличии хоть одного не-`SUBMITTED` | positive+negative/integration | Цикл, 3 слайда от 2 спикеров | Проверка до/после отправки последнего `DRAFT`-слайда | Отсутствует, пока есть `DRAFT`; появляется сразу после отправки всех |
| TC-P8-11 | SPEAKER никогда не получает `ALL_SUBMITTED` от сервера, даже когда условие истинно | negative(role)/integration | Условия TC-P8-10 выполнены | SPEAKER `GET /notifications` | `ALL_SUBMITTED` отсутствует в списке SPEAKER — роль гейтится на бэкенде, не только в UI |
| TC-P8-26 | Просроченный дедлайн (в прошлом) продолжает показывать `DEADLINE_APPROACHING` — нет нижней границы | positive(edge)/integration | `DRAFT`-слайд, `deadline` на 5ч в прошлом, цикл `COLLECTING` | SPEAKER `GET /notifications` | Пункт присутствует |
| TC-P8-27 | Дедлайн через 25ч (за порогом 24ч) не показывает `DEADLINE_APPROACHING` | negative(boundary)/integration | `DRAFT`-слайд, `deadline` через 25ч | SPEAKER `GET /notifications` | Пункт отсутствует |
| TC-P8-29/31 | `NEEDS_REVISION` содержит контекст (`weekLabel`+название шаблона+`reviewComment`), исчезает после переотправки | positive/integration | Слайд, отправленный на доработку с комментарием | SPEAKER `GET /notifications` до/после `POST /:id/submit` | Сообщение содержит все 3 части контекста; после переотправки пункт пропадает |
| TC-P8-легаси | Легаси-шаблон (`layoutKind=null`) — `DEADLINE_APPROACHING`/`NEEDS_REVISION`/`CYCLE_ASSEMBLED` работают идентично блочным шаблонам | positive(regression)/integration | Слайд по легаси-шаблону | Полный цикл: создание→дедлайн-условие→доработка→сборка | Все 3 уведомления корректно срабатывают, `layoutKind` не влияет на логику уведомлений |
| TC-P8-cross | Изоляция между спикерами — спикер B никогда не видит `DEADLINE_APPROACHING`/`NEEDS_REVISION` спикера A | negative/integration | 2 спикера с разными live-условиями в одном цикле | Спикер B `GET /notifications` | В списке спикера B — ноль чужих live-пунктов |

### Мутации `POST /notifications/:id/read`, `/read-all`, `/:id/hide` (Этап 8)

| ID | Название | Категория | Предусловия | Шаги | Ожидаемый результат |
|---|---|---|---|---|---|
| TC-P8-16 | `unreadCount` учитывает непрочитанные персистентные уведомления | positive/integration | Свежее `CYCLE_ASSEMBLED` | `GET /notifications` | `unreadCount >= 1` |
| TC-P8-17/18 | Чужое уведомление → 404 и на `/read`, и на `/hide` (плоская проверка `recipientId`, без ADMIN-исключения) | negative/integration | Уведомление спикера A | Спикер B `POST /:id/read` и `/:id/hide` на чужом id | 404 на оба |
| TC-P8-19 | Владелец отмечает своё уведомление прочитанным | positive/integration | Непрочитанное уведомление | `POST /:id/read` | 200, `readAt` задан |
| TC-P8-20/21 | Скрытие уведомления — мягкое (`hiddenAt`), пропадает из `GET /` | positive/integration | Прочитанное уведомление | `POST /:id/hide` → `GET /` | 200, `hiddenAt` задан в БД, элемент отсутствует в списке |
| TC-P8-22 | `POST /:id/read` на уже СКРЫТОМ уведомлении (после `/hide`) — не 404, обновляет `readAt` тихо, элемент остаётся невидимым в списке | observation/integration | Скрытое уведомление | `POST /:id/read` на id, уже имеющем `hiddenAt` | 200 (не 404) — owner-проверка не учитывает `hiddenAt`; не влияет на видимость, т.к. `GET /` всё равно фильтрует по `hiddenAt: null` — задокументировано как наблюдение, не как дефект (спека не требует блокировки) |
| TC-P8-24/25 | `POST /read-all` — массовая отметка всех непрочитанных | positive/integration | ≥1 непрочитанное персистентное уведомление | `POST /read-all` → `GET /` | 204, затем `unreadCount === 0` |
| TC-P8-race | 5 параллельных `POST /:id/hide` на один и тот же id | positive(edge)/integration | Персистентное уведомление | `Promise.all` из 5 одновременных запросов | Все 5 отвечают 200, без 500/дедлока |
| TC-P8-audit | Все 3 мутации пишут `AuditLogEntry` (`NOTIFICATION_READ`/`_READ_ALL`/`_HIDE`) | positive/integration | — | Прямая проверка БД после вызовов | Все 3 типа действий присутствуют с верным `userId`/`targetId` |

### `NotificationBell` — deep-link и рендер (Этап 8, e2e)

| ID | Название | Категория | Предусловия | Шаги | Ожидаемый результат |
|---|---|---|---|---|---|
| TC-P8-UI-08/09/10 | Колокольчик виден, панель открывается по клику и закрывается по backdrop | positive/e2e | Вход ADMIN | Клик по колокольчику → клик вне панели | Панель открывается/закрывается, счётчик виден |
| TC-P8-deep-1 | Прямой переход по `/slides?cycle=&template=` — `<select>` реально предвыбирают оба значения | positive/e2e | Активное `DEADLINE_APPROACHING`/`NEEDS_REVISION` уведомление спикера | `page.goto('/slides?cycle=X&template=Y')` напрямую (без клика по колокольчику) | `#cycle` и `#template` имеют `value === X/Y` соответственно |
| TC-P8-deep-2 | `/presentation?cycle=` на `ASSEMBLED`-но-НЕ-`ARCHIVED` цикле — работает корректно | positive/e2e | Цикл `ASSEMBLED`, не архивирован | `page.goto('/presentation?cycle=X')` | `#cycle` имеет `value === X`, презентация отображается |
| **TC-P8-deep-3 (ДЕФЕКТ, см. лог прогона)** | `/presentation?cycle=` на `ARCHIVED`-цикле для SPEAKER — дедлайн-переход всегда ведёт в тупик | negative/e2e | `CYCLE_ARCHIVED`-уведомление спикера (цикл уже `ARCHIVED`) | Клик по уведомлению «...архивирована» ИЛИ прямой `page.goto('/presentation?cycle=<archived>')` | Ожидалось: страница показывает архивную презентацию или явную единственную причину недоступности. Факт: `<select>` цикла пуст (ARCHIVED-циклы исключены из списка SPEAKER), ОДНОВРЕМЕННО показаны два противоречащих текста — «Презентация архивирована и недоступна» и «Презентация ещё не собрана» |

### Unit / статическая проверка типов (Этап 8)

| ID | Название | Категория | Шаги | Ожидаемый результат |
|---|---|---|---|---|
| TC-P8-U01 | `tsc --noEmit` чистый после правок Этапа 8 | unit | `npx tsc --noEmit` в `server/` и в `client/` | Без ошибок |

### `POST /api/presentations/cycle/:weeklyCycleId/disassemble` (Этап 9 — симметричная разборка `ASSEMBLED → COLLECTING`)

| ID | Название | Категория | Предусловия | Шаги | Ожидаемый результат |
|---|---|---|---|---|---|
| TC-P9-01 | Экспорт PDF всей презентации ДО разборки (санity, база для сравнения с TC-P9-19) | positive/integration | Собранная презентация | ADMIN `GET /cycle/:id/export.pdf` | 200 |
| TC-P9-02 | Регрессия: `PATCH /weekly-cycles/:id {status}` по-прежнему 400 даже при наличии 3-го статус-меняющего роута | negative(regression)/integration | ADMIN | `PATCH {status: "COLLECTING"}` | 400 |
| TC-P9-03 | SPEAKER вызывает disassemble | negative(role)/integration | — | SPEAKER `POST .../disassemble` | 403 |
| TC-P9-04 | Disassemble несуществующего цикла | negative/integration | ADMIN | `POST /cycle/<random-uuid>/disassemble` | 404 |
| TC-P9-05 | Disassemble `COLLECTING`-цикла (презентации никогда не было) | negative/integration | Свежий цикл | ADMIN `POST .../disassemble` | 409 |
| TC-P9-06/07 | Владелец с 2 слайдами в цикле, в презентации только 1 — уведомление `CYCLE_DISASSEMBLED` ровно одно (не по числу слайдов) | positive/integration | Цикл, 1 из 2 слайдов владельца добавлен | ADMIN `POST .../disassemble` → SPEAKER `GET /notifications` | 200; ровно +1 `CYCLE_DISASSEMBLED` для владельца |
| TC-P9-08/09 | После disassemble: слайд, БЫВШИЙ в презентации → `SUBMITTED`; слайд, НЕ бывший в презентации (тот же владелец, тот же цикл) → не тронут, остаётся `SUBMITTED` | positive/integration | То же | ADMIN `GET /slides/cycle/:id` | Оба `SUBMITTED`, но только по факту, не по побочному эффекту массового `updateMany` |
| TC-P9-10/11 | Chart-image (Этап 7) грузится на `DRAFT`-слайд ДО submit/сборки, доступен по `GET /uploads/chart-images/:assetId` | positive/integration | Слайд по шаблону с `CHART_IMAGE`-блоком | SPEAKER upload → ADMIN `GET` ассета | 200 на upload, 200 на чтение |
| TC-P9-12/13/14/15 | Полный round-trip: сборка (3 реальных слайда: legacy + `CHART_IMAGE`-блочный + `QUADRANT` + 1 заглушка) → disassemble | positive/integration | Цикл с 2 спикерами, 3 слайда submitted, презентация собрана | ADMIN `POST .../disassemble` | 200, `status: COLLECTING`; все 3 реальных слайда → `SUBMITTED` |
| TC-P9-16/17 | `GET /presentations/cycle/:id` после disassemble | positive/integration | То же | ADMIN `GET` | `presentation: null`; `candidateSlides` содержит все 3 вернувшихся слайда |
| TC-P9-18 | Chart-image актива по-прежнему доступен ПОСЛЕ disassemble (владелец) | positive/integration | То же | SPEAKER `GET /uploads/chart-images/:assetId` | 200 — `ChartImageAsset`/файл не тронуты разборкой (ключ — `SlideBlockValue`, не `PresentationSlide`) |
| TC-P9-19 | Экспорт PDF ПОСЛЕ разборки — регрессия на тот же 502-баг класс, что был в Этапе 6 (TC-P6-14) | negative/integration | То же | ADMIN `GET /cycle/:id/export.pdf` | 404 `{"error":"Презентация ещё не собрана"}` — НЕ 502, правильное сообщение (существующий код `presentationExport.ts` уже проверяет `presentation == null` до рендера) |
| TC-P9-20 | Значение `CHART_IMAGE`-блока (`{path: assetId}`) НЕ сброшено разборкой | positive/integration | То же | Повторный `GET /slides/:id` от владельца | `blockValues` для `CHART_IMAGE`-блока — тот же `assetId`, что и до разборки |
| TC-P9-21..25 | Пересборка после disassemble с ДРУГИМ набором/порядком слайдов (пропущен 1 ранее собранный, добавлена новая заглушка) — нет утечки старых слотов/`order` | positive/integration | Цикл из TC-P9-12 | ADMIN добавляет 2 из 3 прежних кандидатов + новую заглушку | Новая презентация — ровно 3 свежих слота с `order` `0,1,2` (не переиспользует старые значения); пропущенный слайд остаётся кандидатом, отсутствует в новой презентации |
| TC-P9-26 | Пересборка после disassemble шлёт НОВОЕ `CYCLE_ASSEMBLED` (не no-op) | positive/integration | То же | SPEAKER `GET /notifications` до/после пересборки | Счётчик `CYCLE_ASSEMBLED` для цикла увеличился — повторная сборка после полного удаления `Presentation` неотличима от первой сборки нового цикла |
| TC-P9-27/28 | Disassemble `ARCHIVED`-цикла (архивация — не расширяется на un-archive в Этапе 9) | negative/integration | Цикл заархивирован после пересборки | ADMIN `POST .../disassemble` | 409 — архивные циклы разбираются не по этой кнопке, вообще не в объёме Этапа 9 |
| TC-P9-29/30/31 | Полный round-trip chart-image: disassemble → пересборка — картинка доступна в контексте новой презентации, файл на диске цел | positive/integration | Слайд с картинкой, разобран и добавлен заново | ADMIN `GET /presentations/cycle/:id`, `GET /uploads/chart-images/:assetId` | Тот же `assetId` виден в новом слоте; файл по-прежнему раздаётся (200) |
| TC-P9-32/33/34 | Аудит: ровно 1 строка `PRESENTATION_DISASSEMBLE` на реальный (не проигравший гонку) вызов, `details` = «слайдов: N, заглушек: M» с верными числами (проверено для N/M = 1/0, 3/1, 1/0) | positive/integration+db | Разборки из TC-P9-06, TC-P9-12, TC-P9-29 | Прямой запрос к БД (`AuditLogEntry`) | По 1 строке на каждый вызов, `details` совпадает с фактическим составом |
| TC-P9-35/36 | **Самоисцеление аномалии**: `WeeklyCycle.status = ASSEMBLED`, но `Presentation` физически отсутствует (аномалия сконструирована напрямую в БД, по классу инцидента «2026-W29», Этапы 5а/6) | positive(edge)/integration | Цикл создан напрямую в БД с `status: ASSEMBLED`, без `Presentation` | ADMIN `POST .../disassemble` | 200 (не 409, не 500), `status → COLLECTING`; аудит `PRESENTATION_DISASSEMBLE` с `details: "слайдов: 0, заглушек: 0"`; уведомлений 0 (в цикле нет ни одного слайда/владельца) |
| TC-P9-37/38/39 | Disassemble цикла, где в презентации была ТОЛЬКО заглушка (0 реальных слайдов), но в самом цикле есть SUBMITTED-слайд, никогда не добавлявшийся | positive(edge)/integration | Цикл: 1 заглушка в презентации, 1 слайд-кандидат вне презентации | ADMIN `POST .../disassemble` | 200; слайд-кандидат остаётся `SUBMITTED` (не тронут); его владелец ВСЁ РАВНО получает `CYCLE_DISASSEMBLED` (получатели — все владельцы слайдов цикла, та же логика, что и `CYCLE_ASSEMBLED`/`CYCLE_ARCHIVED`, не только участники презентации) |
| TC-P9-40/41 | Гонка — 2 одновременных disassemble одного и того же цикла | positive(edge)/integration | `ASSEMBLED`-цикл с 1 слайдом | `Promise.all` 2 параллельных `POST .../disassemble` (разные админ-сессии) | Ровно один 200, один 409; ровно ОДНО `CYCLE_DISASSEMBLED`-уведомление (не два) |
| TC-P9-RACE-0..4 | Гонка — disassemble одновременно с `POST .../slides` (добавление второго слайда) на один и тот же цикл, 5 независимых повторов | positive(edge)/integration | `ASSEMBLED`-цикл с 1 слотом, второй `SUBMITTED`-слайд ещё не добавлен | `Promise.all` (disassemble, add-slide) | Оба запроса детерминированно сериализуются через `SELECT...FOR UPDATE` на `WeeklyCycle`: во всех 5 повторах add-slide неизменно оказывается «после» disassemble (пересобирает презентацию заново) — слайд оказывается либо `IN_PRESENTATION` с реальной подтверждённой строкой `PresentationSlide` (проверено `GET /presentations/cycle/:id`), либо (в принципе) остаётся `SUBMITTED` — но НИКОГДА не наблюдался осиротевший `IN_PRESENTATION` без строки |

### `NotificationBell` / `CyclesPage` — UI разборки (Этап 9, e2e)

| ID | Название | Категория | Предусловия | Шаги | Ожидаемый результат |
|---|---|---|---|---|---|
| TC-P9-E01/E02/E03/E04 | Кнопка «Разобрать» видна только при `status === ASSEMBLED`, сосуществует с «В архив», рядом статичная подпись `.hint-text` (без confirm-диалога) | positive/e2e | ADMIN залогинен, `ASSEMBLED`-цикл в списке | `/cycles` | Обе кнопки видны одновременно; подпись присутствует и упоминает и возврат слайдов, и удаление заглушек |
| TC-P9-E05/E06/E07 | Клик «Разобрать» реально переводит строку в «Сбор», обе кнопки исчезают, состояние переживает `page.reload()` | positive/e2e | То же | Клик «Разобрать» → `reload()` | Бейдж «Сбор» до и после перезагрузки, кнопок нет |
| TC-P9-E08/E09/E10 | Регрессия для SPEAKER: слайд, вернувшийся в `SUBMITTED` после разборки, корректно отражается на `/slides` (бейдж «На проверке», форма read-only, без кнопки «Сохранить») — deep-link `?cycle=&template=` работает и после разборки | positive(regression)/e2e | Слайд владельца был `IN_PRESENTATION`, разобран | `page.goto('/slides?cycle=X&template=Y')` от имени SPEAKER | `<select>` предвыбраны, бейдж «На проверке», активной кнопки «Сохранить» нет |
| TC-P9-E11/E12 | `NotificationBell` — `CYCLE_DISASSEMBLED` виден владельцу слайда, клик не вызывает переход (`targetUrl → null`, прецедент `CYCLE_ARCHIVED`) | positive/e2e | То же | SPEAKER открывает колокольчик → клик по пункту | Сообщение с `weekLabel` видно; URL страницы не меняется после клика |
| TC-P9-E13/E14 | Регрессия `AssemblePage.tsx` (0 правок кода в Этапе 9) — после disassemble+`reload()` корректно показывает «Презентация ещё не собрана» и возвращённый слайд в списке «Готовые слайды» | positive(regression)/e2e | `ASSEMBLED`-цикл с 1 слотом, открыт на `/presentation?cycle=X` | Disassemble через API (имитация «зависшей вкладки») → `page.reload()` той же страницы | Сообщение «ещё не собрана» видно; слайд снова кандидат с кнопкой «Добавить» |

### `GET /api/slides/:id/history` и `GET /api/templates/:id/versions` — история значений/версий (Этап 10)

Префикс `TC-P10-T-*` — независимый прогон `tester`-агента Этапа 10 (просмотр истории значений
поля/блока слайда + реальная история версий шаблона, `GET /api/slides/:id/history` и `GET
/api/templates/:id/versions` в `server/src/routes/slides.ts`/`templates.ts`, новая модель
`TemplateVersion`, `client/src/components/SlideHistoryPanel.tsx`, бейдж «версия N» + «История
версий» в `client/src/pages/TemplatesPage.tsx`). Требования — `REQUIREMENTS.md`, запись
«2026-07-22 — Этап 10».

| ID | Название | Категория | Предусловия | Шаги | Ожидаемый результат |
|---|---|---|---|---|---|
| TC-P10-T-01/02 | Создание легаси-шаблона создаёт версию 1 + 1 снимок | positive/integration | ADMIN | `POST /templates {fields:[...]}` → `GET /:id/versions` | `version===1`, ровно 1 снимок |
| TC-P10-T-04..08 | Реальные изменения значений полей слайда пишут историю, no-op — нет; сортировка/резолв `changedBy` | positive/integration | Легаси-слайд с 2 полями | `PATCH values` (реальное×2, no-op×1) → `GET /:id/history` | Ровно 2+1=3 записи (не 4), сортировка по убыванию `changedAt`, `changedBy.fullName` разрешён (не «?») |
| TC-P10-T-09..12 | **Легаси-пробел (сознательно не устранён, REQUIREMENTS п.5/11)**: добавление НОВОГО поля к легаси-шаблону, у которого уже есть слайд с реальными значениями | positive/integration | Легаси-шаблон + 1 слайд с заполненными значениями | ADMIN `PATCH {fields: [...старые, +новое]}` | 200 (НЕ 409, в отличие от блочных), версия бампается, новый снимок содержит новое поле в `fieldsSnapshot` |
| TC-P10-T-13..15 | `isShared`-only изменение (легаси) бампает версию; no-op повторный `PATCH` с идентичным содержимым — нет | positive+negative(edge)/integration | Легаси-шаблон | `PATCH {isShared:true}` → тот же `PATCH` с идентичным содержимым | Первый бампает версию и создаёт снимок; второй — нет (версия и число снимков не растут) |
| TC-P10-T-16 | Order-only reorder полей легаси-шаблона (без слайдов) бампает версию | positive/integration | Легаси-шаблон, 2 поля, 0 слайдов | `PATCH {fields: [...переставленный order...]}` | Версия увеличивается |
| TC-P10-T-17..22 | Блочный шаблон: создание → версия 1; реальное изменение `TABLE`-блока пишет историю с корректными `oldValue`/`newValue` (массив строк), рендерится/сериализуется без ошибок | positive/integration | Блочный шаблон (`RICH_TEXT_SECTION`/`METRIC_TILE`/`TABLE`/`CHART_IMAGE`) | `POST /templates` → слайд → `PATCH blockValues` (добавление строки в таблицу) → `GET /:id/history` | `version===1`; история содержит запись `kind:"block", label:"Таблица"` с полными `rows` до/после |
| TC-P10-T-23..25 | `CHART_IMAGE`: ADMIN загружает картинку на DRAFT-слайд SPEAKER (не свой) | positive/integration | Блочный слайд SPEAKER, DRAFT, цикл COLLECTING | ADMIN `POST /slides/:id/blocks/:blockId/chart-image` (multipart) | 200; `BlockValueHistory` создаётся с `changedBy` = ADMIN |
| TC-P10-T-24/24b/24c | **Открытый вопрос REQUIREMENTS п.2 — эмпирическая проверка видимости `changedBy`**: SPEAKER читает историю СВОЕГО слайда после правки ADMIN | positive/integration | То же, после загрузки картинки ADMIN'ом | SPEAKER `GET /:id/history` | `changedBy.id`/`changedBy.fullName` в ответе SPEAKER'у **точно равны** id/ФИО администратора — не скрыты и не замаскированы |
| TC-P10-T-26..29 | Полный цикл `CHART_IMAGE` (загружено→заменено→удалено) даёт ровно 3 события истории с правильным текстом; слитый список без дублей | positive/integration | То же | upload → upload (replace) → delete → `GET /:id/history` | 3 отдельных записи `oldValue.path`/`newValue.path`, ни одна не потеряна/задвоена |
| TC-P10-T-30/31 | Слитый список `GET /:id/history` (fields+blocks вместе) — единая сортировка по убыванию, у каждого элемента разрешённый `changedBy.fullName` | positive/integration | Слайд с 7 разнородными записями истории (fields+blocks+chart-image от 2 разных пользователей) | `GET /:id/history` | Список отсортирован целиком, ни одного «?» |
| TC-P10-T-32..37 | **Фокус-зона 1 (замороженный блочный шаблон)**: `PATCH {blocks}` на шаблоне с ≥1 слайдом — 409 (регрессия Этапа 5б); но `PATCH {isShared}` **без ключа `blocks`** на том же шаблоне — 200, версия бампается, новый снимок `blocksSnapshot` точно отражает НЕИЗМЕНЁННЫЕ текущие блоки; повторный no-op `isShared` не бампает | positive+negative(regression)/integration | Блочный шаблон + 1 слайд (заморожен) | `PATCH {blocks:[...]}` → `PATCH {isShared:true}` → `GET /:id/versions` → `PATCH {isShared:true}` (повтор) | 409 / 200+бамп+корректный снимок / без повторного бампа |
| TC-P10-T-38..40 | Order-only reorder блоков и config-only правка `TABLE.columns` (без слайдов, шаблон не заморожен) — оба бампают версию отдельными снимками | positive/integration | Блочный шаблон, 2 блока, 0 слайдов | `PATCH {blocks:[...новый order...]}` → `PATCH {blocks:[...новый config.columns...]}` | 3 снимка всего (create+reorder+config), версии 1→2→3 |
| TC-P10-T-41/41b | `GET /templates/:несуществующий-id/versions` | negative(edge)/integration | ADMIN, случайный UUID | `GET /templates/00000000.../versions` | 200 с пустым массивом (не 404 — эндпоинт не проверяет существование шаблона; отмечено как не заданное явно в REQUIREMENTS, но разумное поведение) |
| TC-P10-T-42/43 | Доступ к `GET /templates/:id/versions`: неаутентифицированный → 401; SPEAKER → 403 | negative(role)/integration | — | `GET /templates/:id/versions` без cookie / с cookie SPEAKER | 401 / 403 |
| TC-P10-T-44..46 | Доступ к `GET /slides/:id/history`: неаутентифицированный → 401; чужой SPEAKER (не владелец) → 404; несуществующий id → 404 | negative(role)/integration | — | `GET /slides/:id/history` в 3 вариантах | 401 / 404 / 404 (existence-hiding, не 403) |
| TC-P10-T-47 | ADMIN читает историю DRAFT (не отправленного) слайда — без статусных ограничений | positive/integration | Слайд `DRAFT` | ADMIN `GET /:id/history` | 200 |
| TC-P10-T-48/49 | История слайда в `ARCHIVED`-цикле (`IN_PRESENTATION`) читается и SPEAKER-владельцем, и ADMIN — без статусных/цикловых ограничений (REQUIREMENTS решение №2) | positive/integration | Легаси-слайд: submit → добавлен в презентацию → цикл заархивирован | SPEAKER и ADMIN `GET /:id/history` | Оба 200 |
| TC-P10-T-50/51 | Регрессия Этапа 7: `checkImageMutationAccess`-гейтинг не сломан (upload на `SUBMITTED`-слайде — 403); чтение истории при этом всё ещё работает | positive(regression)/integration | Блочный слайд, `SUBMITTED` | ADMIN `POST chart-image` / `GET /:id/history` | 403 / 200 |

**E2E (Playwright, `npx playwright@1.61.1`, прямой API без `@playwright/test`)**

| ID | Название | Категория | Предусловия | Шаги | Ожидаемый результат |
|---|---|---|---|---|---|
| TC-P10-T-PW-01/02 | `TemplatesPage`: бейдж «версия N» и раскрывающийся список «История версий» после реальных изменений | positive/e2e | ADMIN, замороженный блочный шаблон с версией 2 (из TC-P10-T-34) | `/templates` → выбрать шаблон → клик «История версий» | Бейдж «версия 2»; список содержит и «версия 1», и «версия 2» с `changedBy`/датой |
| TC-P10-T-PW-03..05 | `ReviewPage`: переключатель истории на карточке слайда; `CHART_IMAGE`-событие рендерится текстом («изображение загружено/заменено/удалено»), НЕ сырым JSON `{"path":...}` | positive/e2e | ADMIN, слайд с полным циклом chart-image-событий | `/review?cycle=X` → раскрыть историю | Текстовые события видны, `{"path"` не встречается на экране |
| TC-P10-T-PW-06/07 | История `TABLE`-блока рендерится (сырым JSON — по дизайну, без diff-алгоритма) без падения страницы/JS-ошибок | positive/e2e | То же | Раскрыть историю, проверить `pageerror` | Данные строк таблицы видны на экране, `page.on("pageerror")` не сработал |

### `GET /api/audit-log` — просмотрщик аудит-лога (Этап 11)

Префикс `TC-P11-T-*` — независимый прогон `tester`-агента Этапа 11 (`server/src/routes/auditLog.ts`,
`client/src/pages/AuditLogPage.tsx`, `client/src/auditActionLabels.ts`). План —
`C:\Users\mille\.claude\plans\glimmering-forging-sky.md`. Основной прогон (31 API + 12 Playwright,
ноль дефектов) уже выполнен основной сессией — не дублируется здесь, см. `SESSION_LOG.md`.

| ID | Название | Категория | Предусловия | Шаги | Ожидаемый результат |
|---|---|---|---|---|---|
| TC-P11-T-01/02 | Матрица доступа: неаутентифицированный → 401, SPEAKER → 403 | negative(role)/integration | — | `GET /audit-log` без cookie / с cookie SPEAKER | 401 / 403 |
| TC-P11-T-03 | Инвертированный диапазон дат (`from` > `to`) | negative(edge)/integration | ADMIN | `GET /audit-log?from=2026-06-01&to=2026-01-01` | 200, `{items:[], hasMore:false, nextCursor:null}` — не 500 |
| TC-P11-T-04 | `LOGIN`/`LOGOUT`-записи: `targetType`/`targetId` действительно `null` в ответе API | positive/integration | ADMIN, существующие `LOGIN`-записи | `GET /audit-log?action=LOGIN` | `targetType===null`, `targetId===null` |
| TC-P11-T-05 | Нераспознанное (будущее) значение `action`/`targetType` возвращается как есть | positive(edge)/integration | Строка `AuditLogEntry` с `action:"SOME_FUTURE_ACTION_NOT_IN_DICTIONARY"`, вставлена напрямую через Prisma | `GET /audit-log?action=SOME_FUTURE_ACTION_NOT_IN_DICTIONARY` | 200, значения `action`/`targetType` не отфильтрованы и не изменены |
| TC-P11-T-06 | **Тай-брейк одинакового `createdAt` (мс)**: 2 строки с идентичным timestamp, постраничный обход `limit=1` через границу | positive(edge)/integration | 2 строки `AuditLogEntry` с одинаковым `createdAt` (вставлены напрямую), разными `id` | `GET` с `from=to=` этот timestamp, `limit=1`, курсор-обход | Обе строки получены ровно по одному разу, без пропуска/дублирования (тай-брейк по `id desc` работает) |
| TC-P11-T-07 | Курсор, полученный под одним фильтром, применён под ДРУГИМ фильтром (не реальный UI-флоу, проверка устойчивости сервера) | positive(edge)/integration | ADMIN | Получить `nextCursor` из безфильтрового запроса → применить его с `action=LOGIN` | 200, корректный ответ (курсор — чистая позиция по `createdAt+id`, не привязан к фильтру) |
| TC-P11-T-08/08b/08c | Некорректный курсор: невалидный base64url / валидный base64url без разделителя `|` / неразбираемая дата в теле курсора | negative(edge)/integration | ADMIN | `GET /audit-log?cursor=...` (3 варианта) | 400 во всех трёх случаях, не 500 |
| TC-P11-T-09 | Некорректные `from`/`to` (не ISO-дата) | negative(edge)/integration | ADMIN | `GET /audit-log?from=not-a-date` / `?to=also-not-a-date` | 400 в обоих случаях |
| TC-P11-T-10 | **Все 4 фильтра одновременно (`action`+`targetType`+`userId`+`from`/`to`) + курсорная пагинация на 2+ страницы** | positive/integration | ADMIN, ≥70 подходящих под фильтр строк | `GET` с `limit=200` (эталон) vs. постраничный обход `limit=1` под тем же составным `where` | Множество id при постраничном обходе точно совпадает с эталоном, без пропусков/дублей |
| TC-P11-T-11 | Клэмпинг `limit`: `limit=0`, `limit=-5` | negative(edge)/integration | ADMIN | `GET /audit-log?limit=0` / `?limit=-5` | Оба откатываются к `DEFAULT_LIMIT=50`, не 0 записей и не ошибка |
| TC-P11-T-12 | Регрессия смежных роутов (`/users`, `/templates`, `/weekly-cycles`, `/notifications`) не затронута изменениями Этапа 11 | positive+negative(regression)/integration | — | Точечные вызовы 401/200/403 на соседних эндпоинтах | Поведение не изменилось |
| TC-P11-T-13 | Длинное/необычное значение `details` (3574 симв., включая `<script>` и кириллицу) не усекается и не модифицируется сервером | positive(edge)/integration | Строка `AuditLogEntry` с длинным `details`, вставлена напрямую | `GET /audit-log` (фильтр по `targetId`) | `details` возвращается побайтово идентичным (длина, спецсимволы, кириллица сохранены) |
| TC-P11-T-14 | Согласованность `hasMore`/`nextCursor` | positive/integration | ADMIN | `GET /audit-log?limit=5` | `hasMore===true` ⇔ `nextCursor!==null` |
| TC-P11-T-EXPLAIN | `EXPLAIN` на 4 вариантах запроса (безфильтровый order/limit, фильтр по `action`, по `targetType`, по `userId`) действительно используют индексы, не seq scan | unit(infra) | `npx prisma` + `$queryRaw`, ~1300 строк в таблице | `EXPLAIN (FORMAT TEXT) SELECT ... ORDER BY ... LIMIT 51` (4 варианта) | Все 4 плана — `Index Scan`/`Bitmap Index Scan` по одному из 4 новых индексов, ни одного `Seq Scan` |

**Playwright**

| ID | Название | Категория | Предусловия | Шаги | Ожидаемый результат |
|---|---|---|---|---|---|
| TC-P11-T-15 | `LOGIN`/`LOGOUT`-строка в таблице рендерит «—» для «Объект»/«ID объекта», без падения страницы | positive/e2e | ADMIN, `/audit-log` | Открыть `/audit-log`, найти строку `LOGIN`/`LOGOUT` | Обе ячейки — текст «—» |
| TC-P11-T-16/16b | Нераспознанный `action`/`targetType` рендерится как есть (fallback на сырой код), не пусто и не падение | positive(edge)/e2e | Строка с `action:"SOME_FUTURE_ACTION_NOT_IN_DICTIONARY"` в БД | Открыть `/audit-log`, долистать «Загрузить ещё» до нужной строки | Ячейка «Действие» = `SOME_FUTURE_ACTION_NOT_IN_DICTIONARY`, ячейка «Объект» = `SomeFutureTargetType` |
| TC-P11-T-17 | **Длинное значение `details` (3574 симв. без пробелов) не ломает вёрстку страницы катастрофически** | negative(defect found)/e2e | Строка с длинным `details` в БД (см. TC-P11-T-13) | Открыть `/audit-log`, снять `document.documentElement.scrollWidth` | ОЖИДАЛОСЬ: ширина страницы остаётся в разумных пределах (обёртка со скроллом или перенос текста). ФАКТ: см. дефект D1 ниже |
| TC-P11-T-17b | Тот же длинный `details` — отсутствие незаэкранированных XSS-значимых символов (React-эскейпинг) | positive/e2e | То же | Проверить `table.data-table script` count в DOM | 0 `<script>`-элементов — текст экранирован корректно |
| TC-P11-T-18 | Ни одной кликабельной ссылки/`<a>` в таблице результатов | positive/e2e | ADMIN, `/audit-log` | Подсчитать `table.data-table a` | 0 |
| TC-P11-T-19 | Неаутентифицированный прямой переход на `/audit-log` | negative(role)/e2e | — | `goto("/audit-log")` без логина | Редирект на `/login`, не пустой экран/падение |

---

## Лог прогонов

### 2026-07-23 — Независимая проверка Этапа 11 (просмотрщик аудит-лога) — найден 1 дефект (вёрстка таблицы при длинном значении `details`)

**Что проверялось:** отдельный от основной сессии реализации проход (основная сессия уже прогнала
31 API + 12 Playwright-проверку, 0 дефектов — см. `SESSION_LOG.md`), нацеленный на 9 зон риска,
явно перечисленных постановщиком задачи как непокрытые основным прогоном: тай-брейк одинакового
`createdAt`, курсор под сменой фильтра, инвертированный диапазон дат, все 4 фильтра + курсор на
2+ страницах, рендер `null`-полей (`LOGIN`/`LOGOUT`), нераспознанные `action`/`targetType`,
регрессия смежных роутов, `EXPLAIN`-проверка использования индексов, длинные/необычные значения
`details`. Реализация — `server/src/routes/auditLog.ts`, `client/src/pages/AuditLogPage.tsx`,
`client/src/auditActionLabels.ts`, план `glimmering-forging-sky.md`.

Прогнано 17 API-проверок (`TC-P11-T-01`…`TC-P11-T-14`, Node/fetch-скрипт, см.
`[[feedback-curl-cyrillic-encoding]]`) + 1 инфраструктурная `EXPLAIN`-проверка + 6 Playwright
e2e-проверок (`TC-P11-T-15`…`TC-P11-T-19`) + `tsc --noEmit` чист на обеих сторонах (сервер и
клиент). Для проверки тай-брейка/нераспознанных значений/длинного `details` тестовые строки
`AuditLogEntry` заводились напрямую через Prisma-скрипт (не через реальные API-мутации — эти
сценарии физически недостижимы через обычный UI-флоу приложения на момент проверки, но
представляют собой валидные HTTP-запросы к уже существующему API, которые сервер обязан
обрабатывать корректно).

**Результат: 23/24 автоматических проверки прошли. Один реальный дефект найден.**

**Фокус-зоны из задания:**
1. **Тай-брейк одинакового `createdAt`.** Две строки `AuditLogEntry` с идентичным `createdAt`
   (мс-точность) и разными `id` — постраничный обход с `limit=1` через границу этих двух строк
   вернул обе ровно один раз, без пропуска/дублирования. Тай-брейк по `id desc` в курсоре работает
   корректно (TC-P11-T-06 — пройден).
2. **Курсор под сменой фильтра.** `nextCursor`, полученный из безфильтрового запроса, успешно
   применён под фильтром `action=LOGIN` — сервер вернул 200 и корректно отфильтрованные строки, без
   ошибок декодирования (курсор действительно чистая позиция `createdAt+id`, не привязан к тому,
   под каким фильтром был выдан — TC-P11-T-07, пройден).
3. **`from > to`.** Возвращает `200 {items:[], hasMore:false, nextCursor:null}`, не 500/исключение
   (TC-P11-T-03, пройден).
4. **Все 4 фильтра + курсор на 2+ страницах.** Составной фильтр (`action=TEMPLATE_CREATE` +
   `targetType=Template` + `userId=<admin>` + широкий диапазон `from`/`to`) на 70 подходящих строках:
   постраничный обход `limit=1` (70 запросов) дал ровно то же множество id, что и единый запрос
   `limit=200` — без пропусков и дублей (TC-P11-T-10, пройден; изначально тест-скрипт сам содержал
   баг — недостаточный `guard`-лимит цикла в 50 итераций при 70 строках, — это баг тестового
   скрипта, не продукта; исправлен и перепрогнан).
5. **`targetId`/`details`, легитимно отсутствующие (`LOGIN`/`LOGOUT`).** Подтверждено и на уровне
   API (`targetType`/`targetId` строго `null`), и визуально в Playwright — ячейки «Объект»/«ID
   объекта» показывают «—», страница не падает (TC-P11-T-04/15, пройдены).
6. **Нераспознанное/будущее значение `action`/`targetType`.** Строка с `action:
   "SOME_FUTURE_ACTION_NOT_IN_DICTIONARY"` и `targetType: "SomeFutureTargetType"` (вставлена
   напрямую через Prisma) — API возвращает оба значения без изменений; UI (`auditActionLabel()`/
   `auditTargetTypeLabel()`) корректно откатывается на сырой код вместо падения/пустой ячейки
   (TC-P11-T-05/16/16b, пройдены).
7. **Регрессия смежных роутов.** `/api/users`, `/api/templates`, `/api/weekly-cycles`,
   `/api/notifications` — поведение (401/200/403) не изменилось после добавления Этапа 11
   (TC-P11-T-12, пройден).
8. **Целостность композитного индекса `(createdAt, id)` и трёх одиночных индексов.** `EXPLAIN
   (FORMAT TEXT)` на 4 вариантах запроса (безфильтровый order+limit, фильтр по `action`, по
   `targetType`, по `userId`, ~1300 строк в таблице на момент проверки) — все 4 плана используют
   один из новых индексов (`Index Scan Backward using "AuditLogEntry_createdAt_id_idx"` для
   безфильтрового и `userId`-фильтра — планировщик на этом объёме данных предпочёл составной индекс
   даже для `userId`-фильтра, что нормально и ожидаемо при небольшом наборе данных; `Bitmap Index
   Scan` по `AuditLogEntry_action_idx`/`AuditLogEntry_targetType_idx` для соответствующих фильтров).
   Ни одного `Seq Scan` ни в одном из 4 планов — индексы реально используются, не просто существуют
   (TC-P11-T-EXPLAIN, пройден).
9. **Длинное/необычное значение `details`.** API возвращает значение (3574 символа, включая
   `<script>alert('xss')</script>` и кириллицу) побайтово без изменений — не усекает и не
   санитизирует на сервере (TC-P11-T-13, пройден). XSS-часть подтверждена безопасной: React
   экранирует текстовое содержимое по умолчанию, `<script>`-элемент в DOM не появляется
   (TC-P11-T-17b, пройден). **Но вёрстка — реальный дефект, см. ниже.**

**Дефект D1 (реальный, требует исправления) — катастрофическое растягивание таблицы/страницы при
длинном значении `details` без пробелов.**
- **Симптом:** одна-единственная запись аудит-лога с длинным неразрывным значением `details` (в
  тесте — 3574 символа, включая непрерывный блок из 3000+ символов без пробелов, что реалистично:
  например, длинный токен, хэш, путь к файлу, вставленный URL или просто длинная фраза без
  пробелов) заставляет `<table class="data-table">` растянуться до ширины содержимого этой ячейки,
  а поскольку вокруг таблицы нет обёртки с `overflow-x: auto`/`overflow: hidden`, растягивается вся
  страница целиком — `document.documentElement.scrollWidth` вырос с ожидаемых ~1280 (ширина
  вьюпорта) до **27900–28152px**. Визуально это выглядит как страница, полностью «уехавшая» вбок:
  все обычные строки таблицы сжаты в узкую полоску шириной в пару процентов экрана, а основная
  ширина уходит на единственную длинную ячейку. Скриншот:
  `C:\Users\mille\AppData\Local\Temp\claude\C--Users-mille-sigir1\ee762fa8-be03-4f6b-928a-8c80ef8d00a5\scratchpad\p11-audit-log-page.png`.
- **Причина (по коду, не для исправления в этом прогоне — только диагностика):**
  `client/src/styles.css`, класс `.data-table td` (строки 496-501) не имеет `word-break`/
  `overflow-wrap`, а класс `.content` (строки 294-298) имеет `max-width: 880px`, но не
  `overflow-x`/обёртку-скролл вокруг таблицы — так что длинная неразрывная строка в любой ячейке
  таблицы (не только `details`) раздувает весь документ, а не остаётся в рамках карточки/страницы.
- **Реалистичность сценария (не чисто синтетический тест):** поле `details` в `AuditLogEntry`
  как минимум в одном реальном пути записи (`server/src/routes/presentations.ts:162` —
  `details: label.trim()`, свободный текст названия заглушки при сборке презентации) заполняется
  из пользовательского текстового поля без ограничения длины ни на клиенте (не найден `maxLength`),
  ни на сервере — то есть администратор, вставивший длинную строку без пробелов в качестве
  названия заглушки при сборке презентации, воспроизведёт эту же поломку вёрстки на `/audit-log`
  без какого-либо прямого доступа к БД.
- **Шаги воспроизведения (через Prisma напрямую, минимально):**
  1. `INSERT INTO "AuditLogEntry" (..., details) VALUES (..., '<3000+ символов без пробелов>')`
     (или косвенно — собрать презентацию с заглушкой, чьё название состоит из очень длинной строки
     без пробелов).
  2. Зайти ADMIN'ом на `/audit-log`.
  3. Наблюдать: `document.documentElement.scrollWidth` вырастает до ~28000px вместо ожидаемых
     ~1280 (ширина вьюпорта); визуально все строки таблицы сжаты в узкую полосу слева, страница
     требует горизонтальной прокрутки на десятки экранов вбок.
- **Не проверялось (вне рамок этого прогона):** пре-существует ли та же брешь в CSS на других
  страницах, использующих `.data-table` (`CyclesPage.tsx`, `slideBlocks.tsx`) — не проверено,
  потому что там (по беглому просмотру) не обнаружено полей со свободным неограниченным
  пользовательским текстом такой же природы; упомянуто как потенциальный смежный риск, не как
  подтверждённый дефект на этих страницах.

**Тестовые данные, оставленные в БД (не удалялись):**
- 1 новый пользователь-спикер: `qa-p11-tester-speaker` (ФИО «QA-P11-TESTER Speaker», id
  `a9bdb8f9-2d29-4a02-a4b0-36ae506c03b1`, роль SPEAKER, создан через `POST /api/users`, пароль
  сброшен через `/users/:id/reset-password` в ходе прогона — последний временный пароль не
  зафиксирован постоянно, известен только в рамках этого прогона).
- 4 синтетические строки `AuditLogEntry`, вставленные напрямую через Prisma (не через обычный
  API-флоу — специально для покрытия недостижимых через UI граничных случаев):
  - `65842c68-726e-4dfc-b071-c2285318a148` (`action:"QA_P11_TIEBREAK_A"`, `createdAt:
    2026-01-15T10:00:00.123Z`) и `5d3ad9d6-109d-48b1-a397-5e82442e6f12`
    (`action:"QA_P11_TIEBREAK_B"`, тот же `createdAt`) — пара для тай-брейк-теста.
  - `d6534749-4221-4a7e-8614-351f427f13ae` (`action:"SOME_FUTURE_ACTION_NOT_IN_DICTIONARY"`,
    `targetType:"SomeFutureTargetType"`, `targetId:"qa-future-1"`) — для проверки fallback-рендера
    нераспознанного значения.
  - `e28765d3-5ac9-4d4b-b303-62364fa1750f` (`action:"USER_PASSWORD_RESET"`, `targetType:"User"`,
    `targetId:"qa-long-details"`, `details` длиной 3574 символа) — для дефекта D1 и XSS-проверки.
- Файлов на диске не оставлено — прогон не создавал вложений/картинок.
- Все вспомогательные Node-скрипты прогона (`qa-p11-*.js` в `server/`) удалены после использования
  — в `git status` после прогона они не отображаются.

**Дефект D1 — исправлен в этой же сессии (сразу после отчёта `tester`-агента).** Первая попытка
(`overflow-wrap: anywhere` на `.data-table td`/`th` без `table-layout: fixed`) устранила
раздувание ширины страницы, но вскрыла смежную проблему: авто-раскладка таблицы сжимала все
остальные колонки до нечитаемой узкой полосы (заголовки вроде «Дата и время» переносились
по одной букве), чтобы выделить максимум места длинной ячейке — обнаружено собственным
Playwright-скриншотом до принятия фикса, не отдельным прогоном `tester`-агента. Итоговое
исправление — `table-layout: fixed` на `.data-table` (строка ширины колонок больше не зависит
от содержимого конкретной ячейки) вместе с `overflow-wrap: anywhere` на `td`/`th` (длинное
значение переносится по буквам внутри уже фиксированной ширины колонки, не наружу). Проверено:
1) прямое воспроизведение репро `tester`-агента (строка с 3574-символьным `details` через Prisma)
— `document.documentElement.scrollWidth` вернулся к нормальным ~1280px, значение при этом
по-прежнему полностью отображается (не обрезано); 2) скриншот `/audit-log` — все колонки читаемы,
длинное значение аккуратно переносится в своей колонке; 3) скриншот `/cycles` (та же CSS-разметка
`.data-table`, единственная другая страница-потребитель) — визуальных регрессий не обнаружено;
4) полный повторный прогон исходных 31 API + 12 Playwright-проверок основной сессии — 0 дефектов,
без изменений в поведении. `tsc --noEmit` не запускался повторно (изменения только в CSS).

**Вывод:** реализация Этапа 11 в основном соответствует плану и требованиям — 8 из 9 запрошенных
фокус-зон подтверждены без отклонений. Один реальный дефект (D1, вёрстка) найден и задокументирован
для исправления основной сессией/разработчиком; сам эндпоинт API и логика пагинации/фильтрации/
курсора остаются корректными во всех проверенных граничных случаях.

---

### 2026-07-23 — Независимая проверка Этапа 10 (история значений поля/блока слайда + реальная история версий шаблона) — дефектов не найдено

**Что проверялось:** отдельный от основной сессии реализации проход, нацеленный на 9 зон риска,
явно перечисленных постановщиком задачи как непокрытые собственным прогоном основной сессии
(happy path создания/no-op/реального изменения версии шаблона, 404/200/403/401-матрица доступа,
CHART_IMAGE round-trip, конкурентность — см. предыдущий контекст задачи), а не на дублирование
уже пройденного. Реализация и требования — `REQUIREMENTS.md`, запись «2026-07-22 — Этап 10».
Прогнано 59 API-проверок (`TC-P10-T-01`…`TC-P10-T-51`, включая объединённые под одним ID шаги
сетапа, Node/fetch-скрипт, см. `[[feedback-curl-cyrillic-encoding]]`) + 7 Playwright
e2e-проверок (`TC-P10-T-PW-01`…`TC-P10-T-PW-07`) + `tsc --noEmit` чист на обеих сторонах.
**Все 66 автоматических проверок прошли, ни одного дефекта не найдено.**

**Фокус-зоны из задания — все подтверждены эмпирически, без отклонений:**
1. **Замороженный блочный шаблон, `PATCH` без ключа `blocks`.** `PATCH {blocks:[...]}` на шаблоне
   с ≥1 слайдом по-прежнему 409 (регрессия Этапа 5б не пострадала). Но тот же шаблон с `PATCH
   {isShared:true}` (без `blocks` в теле вообще) — 200, версия бампается (1→2), новый снимок
   `TemplateVersion.blocksSnapshot` при этом точно совпадает с текущими (неизменными) 4 блоками
   шаблона, включая `CHART_IMAGE`-блок с `config:null` и `TABLE`-блок с реальными `config.columns`.
   Повторный no-op `PATCH {isShared:true}` версию не бампает и снимок не создаёт.
2. **Открытый вопрос REQUIREMENTS.md, п.2 (`changedBy`-видимость).** Эмпирически подтверждено:
   когда ADMIN напрямую грузит/меняет `CHART_IMAGE` на чужом (SPEAKER-owned) слайде, сам SPEAKER
   при собственном вызове `GET /:id/history` видит `changedBy.fullName`/`changedBy.id`,
   ТОЧНО совпадающие с ФИО/id администратора — никакой маскировки/скрытия нет, поведение
   соответствует зафиксированному в REQUIREMENTS решению (не согласованная с заказчиком позиция,
   а осознанный выбор по умолчанию — сама проверка лишь подтверждает, что код делает то, что
   написано в решении).
3. **Легаси-пробел (сознательно не устранён, REQUIREMENTS п.5/11/«Открытые вопросы» п.1).**
   Добавление нового поля к легаси-шаблону, у которого уже есть слайд с реальными сохранёнными
   значениями других полей, — подтверждено, что это по-прежнему 200 (не 409, в отличие от
   аналогичного случая с блочными шаблонами), версия шаблона бампается, а новый снимок
   `fieldsSnapshot` корректно содержит добавленное поле — история версий действительно делает
   этот пробел «видимым», как и было заявлено, ничего не блокируя.
4. **`TABLE`-блок в истории/снимках.** Реальное изменение `TABLE`-значения (добавление строки)
   создаёт ровно одну запись `BlockValueHistory` с полными `oldValue.rows`/`newValue.rows`;
   клиентский `SlideHistoryPanel.renderValue()` (generic `JSON.stringify` для не-строк) не падает
   и показывает читаемые (хоть и сырые) данные — подтверждено и на уровне API, и визуально в
   Playwright-скриншоте `/review`. Отдельно подтверждено: `PATCH`, меняющий только
   `config.columns` у `TABLE`-блока (без изменения `order`/`label`/других блоков), тоже бампает
   версию шаблона отдельным снимком.
5. **`isShared`-only и order-only изменения.** Оба варианта — изменение только `isShared` (легаси
   и блочный, включая замороженный случай из п.1) и переупорядочивание полей/блоков без всякого
   другого изменения (легаси и блочный) — корректно бампают версию шаблона отдельными снимками;
   `templateSnapshotKey()` действительно учитывает `order` внутри `fieldsSnapshotOf`/
   `blocksSnapshotOf`, как и задокументировано.
6. **`GET /templates/:несуществующий-id/versions`.** Возвращает `200 []` (не `404`) — эндпоинт не
   проверяет существование шаблона перед `findMany`. Не описано явно в REQUIREMENTS.md как
   принятое решение, но безопасно (не течёт данными чужого шаблона) и предсказуемо для
   `TemplatesPage.tsx`, который вызывает этот эндпоинт только для уже выбранного из списка
   существующего шаблона — отмечено как наблюдение, не как дефект.
7. **Матрица доступа.** Все 5 явно запрошенных комбинаций подтверждены: неаутентифицированный
   `GET /templates/:id/versions` → 401 (раньше `requireRole`, как и ожидалось от порядка
   мидлварей); SPEAKER на том же эндпоинте → 403; `GET /slides/:id/history` для слайда в
   `ARCHIVED`-цикле по-прежнему 200 и для SPEAKER-владельца, и для ADMIN (решение №2 —
   «без статусных ограничений» — подтверждено буквально на архивном цикле, не только на
   `COLLECTING`); ADMIN читает историю `DRAFT`-слайда (никогда не отправленного) — 200.
8. **Целостность объединённого списка.** На слайде с 7 разнородными записями истории (3
   изменения текстовых/легаси-полей от SPEAKER + 3 CHART_IMAGE-события от ADMIN + 1 изменение
   TABLE от SPEAKER, на двух разных шаблонах/слайдах в сумме) — слитый `GET /:id/history`
   ни разу не терял и не дублировал ни одной записи (сверено по составному ключу
   `kind+label+changedAt+oldValue+newValue`), список целиком отсортирован по убыванию, ни у одной
   записи `changedBy.fullName` не оказался «?».
9. **Регрессия смежных этапов.** Точечно (не полный повтор): `checkImageMutationAccess`
   (Этап 7) по-прежнему возвращает 403 на попытку загрузки картинки на уже `SUBMITTED`-слайд —
   Этап 10 не ослабил и не сломал эту проверку мутации, при этом чтение истории того же слайда
   (`GET /:id/history`) остаётся доступным независимо от статуса — то есть разграничение
   «мутация требует статус, чтение — нет» реализовано именно так, как задокументировано в
   решении №2 REQUIREMENTS.md.

**Playwright, визуальное подтверждение:** скриншот `/templates` с выбранным замороженным блочным
шаблоном показывает одновременно баннер «Заморожено — по шаблону уже есть слайды, состав блоков
менять нельзя», бейдж «версия 2» и раскрытый список «История версий» с обеими записями (версия 1
и версия 2, у каждой — `changedBy`/дата); скриншот `/review` с раскрытой историей слайда
показывает текстовые события для `CHART_IMAGE` («изображение удалено» / «изображение заменено» /
«изображение загружено», в порядке от новых к старым) вперемешку с сырым JSON `{"rows":[...]}`
для `TABLE` — визуально подтверждает решение №3/№8 REQUIREMENTS.md (текстовые события для картинок,
без diff-алгоритма для остального) в реальном UI, не только в API-ответе.

**Дефектов не найдено.** Все 9 запрошенных фокус-зон подтверждены как «работает согласно
REQUIREMENTS.md», включая оба явно помеченных в REQUIREMENTS.md «Открытых вопроса» (видимость
`changedBy` для ADMIN-правок и легаси-пробел с добавлением поля) — оба ведут себя ровно так, как
описано в принятых решениях, что не то же самое, что «согласовано с заказчиком»: сами открытые
вопросы (стоит ли когда-либо это менять) остаются открытыми, эта проверка лишь подтверждает, что
код реализует именно те решения, что записаны.

**Тестовые данные, оставленные в БД (не удалялись):**
- 2 новых пользователя-спикера: `qa.p10.tester.<timestamp>` / `qa.p10.tester2.<timestamp>`
  (созданы через `POST /api/users`, временные пароли зафиксированы только в рамках этого прогона).
- 4 новых `WeeklyCycle` с `weekLabel`, начинающимся на «QA-P10-TESTER cycle» (legacy/block/reorder/
  archive) — один (`cycle legacy`) доведён до статуса `ARCHIVED` в рамках сценария TC-P10-T-48/49,
  остальные три остались в `COLLECTING` (`cycle reorder` фактически не использован ни одним
  слайдом — заведён про запас, не понадобился).
- 4 новых `Template`: «QA-P10-TESTER Legacy Template» (легаси, 3 версии — create/add-field/
  isShared), «QA-P10-TESTER Legacy Reorder» (легаси, 2 версии), «QA-P10-TESTER Block Template»
  (блочный, заморожен 1 слайдом, 2 версии — create/isShared), «QA-P10-TESTER Block Reorder»
  (блочный, без слайдов, 3 версии — create/reorder/table-config).
- 2 новых `Slide` с реальной историей значений (легаси — 3 записи `FieldValueHistory`; блочный —
  1 запись `BlockValueHistory` для TABLE + 3 записи для CHART_IMAGE от ADMIN), плюс связанные
  `PresentationSlide`/`Notification`/`AuditLogEntry`.
- 3 реальных PNG-файла в `server/uploads/chart-images/` (загружено/заменено/удалено в рамках
  TC-P10-T-23..27 — итоговое состояние блока «Картинка» — `path:null`, последний физический файл
  удалён самим тестируемым кодом, как и задокументировано в Этапе 7; первые два файла тоже уже
  удалены тем же механизмом при замене/удалении, на диске от этого прогона реально ничего не
  остаётся).
- Скриншоты Playwright — во временном scratch-каталоге сессии (`
  C:\Users\mille\AppData\Local\Temp\claude\...\scratchpad\pw01_template_versions.png`,
  `pw02_review_history.png`), не в репозитории.

**Вывод:** реализация Этапа 10 полностью соответствует `REQUIREMENTS.md`, включая все зоны риска,
явно указанные постановщиком задачи как непокрытые собственным прогоном основной сессии. Дефектов
не найдено.

---

### 2026-07-22 — Независимая проверка Этапа 9 (симметричная разборка презентации `ASSEMBLED → COLLECTING`) — дефектов не найдено

**Что проверялось:** отдельный от основной сессии реализации проход, нацеленный на пробелы вне
собственного прогона основной сессии (44 API-проверки + 6 Playwright UI-проверок — не
дублируются здесь). Реализация и требования — `REQUIREMENTS.md`, запись «2026-07-22 — Этап 9».
Прогнано 41 нумерованный кейс (`TC-P9-01`…`TC-P9-41`, включая объединённые под одним ID шаги
сетапа) + 5 независимых повторов гонки `TC-P9-RACE-0..4` + 14 Playwright e2e-проверок
(`TC-P9-E01`…`TC-P9-E14`). **Все прошли, ни одного дефекта не найдено** — независимо
подтверждена вся заявленная функциональность, включая оба места конкурентности из
Plan-агентского ревью.

**Особый акцент (по прямому запросу постановщика, вне собственного покрытия основной сессии):**
- **Самоисцеление аномалии** (`ASSEMBLED` без `Presentation`) воспроизведено НЕ обходным путём
  (через реальный API не собрать такое состояние), а прямой ручной записью через Prisma-клиент в
  БД — тот же класс инцидента, что и исторический цикл «2026-W29» (Этапы 5а/6). Результат
  подтверждён и на уровне API (200, `status → COLLECTING`), и на уровне БД (ровно 1
  `AuditLogEntry` с `details: "слайдов: 0, заглушек: 0"`, 0 строк `Notification` — в цикле не
  было ни одного слайда/владельца, так что `notifyCycleSlideOwners` корректно не создал ничего).
- **Chart-image (Этап 7) round-trip через disassemble** — загружено реальное PNG на `CHART_IMAGE`-
  блок ДО submit, затем полный цикл submit → сборка → disassemble → пересборка. Значение
  `{path: assetId}` не сброшено ни на одном шаге, файл физически остаётся раздаваемым (`GET
  /uploads/chart-images/:assetId` → 200) после каждого шага — подтверждает, что механизм
  разборки не касается `SlideBlockValue`/`ChartImageAsset`, как и заявлено.
- **Полный round-trip с другим набором/порядком слайдов**: собрано (3 реальных слайда + 1
  заглушка) → разобрано → пересобрано С ДРУГИМ подмножеством (2 из 3 прежних слайдов + новая
  заглушка, один прежний слайд сознательно пропущен). Новая презентация содержит РОВНО 3 свежих
  слота с `order` `0,1,2` (не переиспользует старые значения `order`, не тянет пропущенный слайд),
  пропущенный слайд остаётся кандидатом. Новое `CYCLE_ASSEMBLED`-уведомление реально создаётся
  при пересборке (не дедуплицируется как no-op) — прямое следствие полного `delete` презентации,
  а не мягкой пометки, как и обосновано в REQUIREMENTS.md.
- **PDF-экспорт сразу после disassemble** — независимо перепроверен путь, который в Этапе 6 был
  источником реального бага (потеря сообщения об ошибке, общий 502 вместо содержательного ответа,
  TC-P6-14). После disassemble `GET /cycle/:id/export.pdf` корректно отвечает `404
  {"error":"Презентация ещё не собрана"}`, не 502 — регрессии не найдено, `presentationExport.ts`
  действительно не потребовал изменений, как и заявлено.
- **Уведомление `CYCLE_DISASSEMBLED` для владельца с несколькими слайдами в цикле, только часть
  которых была в презентации** — проверено дважды: (а) владелец с 2 слайдами, в презентации был
  только 1 — ровно ОДНО уведомление, не два; (б) владелец с 1 SUBMITTED-слайдом, который вообще
  никогда не добавлялся в презентацию (в презентации была только заглушка) — владелец ВСЁ РАВНО
  получает уведомление, подтверждая, что получатели вычисляются по всем слайдам цикла, а не по
  фактическому составу презентации (та же семантика, что и у `CYCLE_ASSEMBLED`/`CYCLE_ARCHIVED`
  из Этапа 8).
- **Независимая перепроверка обеих гонок из Plan-агентского ревью**: (1) 2 одновременных
  disassemble одного цикла — ровно один 200/один 409, ровно одно уведомление (не два); (2) 5
  независимых повторов гонки disassemble-vs-add-slide (второй `SUBMITTED`-слайд добавляется в
  презентацию конкурентно с разборкой того же цикла) — во всех 5 повторах `SELECT...FOR UPDATE`
  на `WeeklyCycle` корректно сериализовал операции (оба запроса неизменно завершались успехом, но
  строго последовательно: add-slide каждый раз фактически пересобирал презентацию заново уже
  после того, как disassemble её удалил), финальное состояние слайда всегда консистентно
  (`IN_PRESENTATION` с реальной строкой `PresentationSlide`, подтверждено прямым `GET`) — ни разу
  не воспроизведён осиротевший `IN_PRESENTATION` без строки.
- **Полная регрессия `AssemblePage.tsx` (0 правок кода в этом этапе) и спикерского `/slides`** —
  оба подтверждены в реальном браузере (не только через API): (а) ADMIN на `/presentation`,
  открытом на цикле ДО разборки, при `reload()` после disassemble (эмуляция «зависшей вкладки»)
  корректно видит «Презентация ещё не собрана» и вернувшийся слайд в списке кандидатов; (б)
  SPEAKER на `/slides?cycle=&template=` (в т.ч. через deep-link) видит бейдж «На проверке» и форму
  без активной кнопки «Сохранить» для слайда, который до разборки был `IN_PRESENTATION` — ни один
  из этих экранов не потребовал правок, что и было заявлено в REQUIREMENTS.md.

**Наблюдение вне рамок Этапа 9 (не дефект этого этапа):** обнаружена испорченная кириллица
(mojibake — реально повреждённые байты в БД, не артефакт отображения/консоли) в `name`/`label`
существующего dev-легаси-шаблона (id `230542d2-0cac-4708-b605-c664d1c195d5`, `createdAt:
2026-07-18`, до начала работы над Этапом 9 и вообще до документирования известной особенности
машины про `curl -d`+кириллицу). Подтверждено на уровне API-ответа (JSON, записанный в файл
напрямую в UTF-8, показывает те же испорченные байты) и визуально на скриншоте Playwright —
значит это не проблема кодировки терминала/консоли, а реальные испорченные байты, попавшие в БД
при более раннем создании этого шаблона (скорее всего, тем же классом причины, что и
задокументированная в памяти проекта проблема `curl -d` с кириллицей, просто на несколько дней
раньше, чем она была явно поймана и задокументирована). Функционально это НЕ помешало ни одной
проверке Этапа 9 (все API/e2e-кейсы, использовавшие этот шаблон, прошли корректно — сама разборка
не работает с текстовым содержимым полей). Зафиксировано как информация для будущих сессий — этот
конкретный шаблон непригоден как «чистый» источник кириллического текста для тестов, отображающих
поля пользователю; для новых тестов лучше заводить свежие шаблоны через API теста, а не
переиспользовать этот.

**Тестовые данные, оставленные в БД (не удалялись):**
- 2 новых пользователя-спикера: `qa.speaker.p9a` / `qa.speaker.p9b` (созданы через `POST
  /api/users`, временные пароли зафиксированы только в рамках этого прогона, не в этом файле).
- 17 новых `WeeklyCycle` с `weekLabel`, начинающимся на `QA-P9-` (метки времени
  `1784743369781`…`1784744447275`), в статусах `ARCHIVED`/`ASSEMBLED`/`COLLECTING` в разных
  комбинациях — по одному на каждый сценарий выше (round-trip, self-heal-аномалия, гонки ×2 вида,
  chart-image round-trip, edge-кейсы с одними заглушками, e2e-проверки). Ранее существовавшие
  `QA-P9-*`-циклы с метками `1784740…` в БД — не мои, оставлены основной сессией реализации при
  её собственном 44-проверочном прогоне, не трогались.
- Несколько новых `Slide`/`PresentationSlide`/`Notification`/`AuditLogEntry` строк, связанных с
  этими циклами — не удалялись.
- 1 файл `ChartImageAsset` на диске (`server/uploads/chart-images/`, реальный 1×1 PNG, id
  `0f4888dc-ddae-4abb-b76b-7a5988f68a5c`) — переживший полный round-trip disassemble/пересборки,
  оставлен как есть.
- Один временный `WeeklyCycle` создан НАПРЯМУЮ в БД (в обход API) для воспроизведения
  self-heal-аномалии (`status: ASSEMBLED` без `Presentation`, id `0c3bb557-c9f8-4f1e-b773-
  a53a2c21859c`) — уже самоисцелён вызовом `disassemble` в рамках этого же прогона, сейчас в
  статусе `COLLECTING`, дальнейших остаточных аномалий не оставляет.
- Скриншоты Playwright — во временном scratch-каталоге сессии (`
  C:\Users\mille\AppData\Local\Temp\claude\...\scratchpad\p9_speaker_after_disassemble.png`,
  `p9_assemble_after_disassemble.png`), не в репозитории.

**Вывод:** реализация Этапа 9 полностью соответствует `REQUIREMENTS.md`, включая обе
концурентные доработки, найденные независимым Plan-агентским ревью. Дефектов не найдено ни в
одной из областей, явно указанных постановщиком задачи как непокрытые собственным прогоном
основной сессии.

### 2026-07-22 — Независимая проверка Этапа 8 (внутриприложенческие уведомления) — найдено 2 дефекта

**Что проверялось:** отдельный от основной сессии реализации проход, нацеленный на пробелы, явно
перечисленные постановщиком задачи как НЕ покрытые собственным тестированием основной сессии (её
собственный прогон — 44 API-проверки + 6 Playwright UI-проверок, см. её описание задачи; сюда не
дублируется). Реализация — план `C:\Users\mille\.claude\plans\tidy-forging-dahl.md`, требования —
`REQUIREMENTS.md`, секция «Этап 8» (полностью согласована, включая финальный подраздел «Ответы
пользователя»). Дев-сервер/клиент уже были подняты (`127.0.0.1:4000`/`127.0.0.1:5173`), последняя
миграция `20260721200930_add_notifications` уже применена.

**Инструменты:** Node 24 (встроенный `fetch`) для API-скриптов (не `curl -d`, см.
`[[feedback-curl-cyrillic-encoding]]`), `npx playwright@1.61.1` (браузер уже закэширован,
`chromium-1228`) для UI-скриптов, оба временно запускались из `server/` для резолва пакетов;
сами скрипты временно копировались в `server/` для запуска и удалены после прогона (не
коммитятся). Скриншоты сохранены в scratch-каталоге сессии, не в репозитории. Прямые SQL/Prisma
запросы к БД — для проверки утверждений, не проверяемых напрямую через API (число строк
`Notification`, hex-дамп мангл-данных, `AuditLogEntry`).

**1. Дублирование `Notification`-строк для одного владельца с несколькими слайдами в одном цикле
— НЕ подтверждено, работает как задумано.** Спикер с 2 слайдами (разные шаблоны) в одном цикле —
после сборки+архивации в БД ровно по 1 строке `Notification` на `(recipientId, type)`, не 2 (не по
одной на слайд). Проверено прямым запросом Prisma (`TC-P8-DB-1`).

**2. «Слайд, не попавший в презентацию, всё равно уведомляется» — подтверждено.** Владелец слайда,
который остался `DRAFT` (никогда не отправлялся, никогда не участвовал в сборке), получил
`CYCLE_ASSEMBLED` в тот же момент, что и владелец добавленного слайда — оба воспринимаются как
«владельцы слайдов этого цикла» (`TC-P8-13/14`).

**3. Живые условия — границы дедлайна, «все сданы», «на доработке» — все прошли без замечаний.**
Полная матрица: `DRAFT` в пределах 24ч показывает `DEADLINE_APPROACHING`, `SUBMITTED` — нет;
просроченный дедлайн (−5ч) продолжает показываться (нет нижней границы); дедлайн через +25ч не
показывается; `ALL_SUBMITTED` появляется только когда абсолютно все существующие слайды цикла
`SUBMITTED` (≥1), пропадает сразу после перехода цикла в `ASSEMBLED`; `NEEDS_REVISION` содержит
weekLabel+шаблон+комментарий, пропадает после переотправки. Кросс-спикерная изоляция — спикер B
никогда не видит live-условия спикера A. Ролевой гейт `ALL_SUBMITTED` подтверждён и на прямом
вызове API спикером (не только в UI) — сервер никогда не возвращает этот тип SPEAKER-роли.

**4. Read/hide/read-all — включая гонки — без замечаний, кроме одного наблюдения.** Кросс-
пользовательские 404 на `/read` и `/hide` подтверждены. 5 параллельных `POST /:id/hide` на один и
тот же id — все отвечают 200, без 500/дедлока. Все 3 мутации пишут `AuditLogEntry` с верными
`userId`/`targetId`. Единственное наблюдение (не дефект, задокументировано как `TC-P8-22`):
`POST /:id/read` на уже скрытом (`hiddenAt` задан) уведомлении возвращает 200 и тихо обновляет
`readAt`, а не 404 — проверка владения не учитывает `hiddenAt`. Это не противоречит явной спеке
(нигде не сказано, что скрытие должно блокировать последующую отметку прочитанным), и не влияет на
видимость в списке (`GET /` всё равно фильтрует `hiddenAt: null`), поэтому не поднимаю это как
дефект, только фиксирую как пограничный случай на будущее.

**5. Легаси-шаблон (`layoutKind = null`) — полный регресс без замечаний.** Слайд по легаси-шаблону
прошёл весь цикл (создание → `DEADLINE_APPROACHING` → отправка → доработка с `NEEDS_REVISION` →
переотправка → сборка через заглушку → `CYCLE_ASSEMBLED`) идентично блочным шаблонам — `layoutKind`
не влияет на логику уведомлений, как и ожидалось по плану.

**Дефект №1 (реальный, подтверждён и воспроизведён) — `CyclesPage.tsx`: дедлайн «сдвигается» на
величину часового пояса сервера/браузера при повторном открытии инлайн-редактирования, вплоть до
тихого искажения значения при пересохранении без касания самого поля дедлайна.**
`toDateTimeInputValue(iso)` (используется для заполнения `<input type="datetime-local">` при клике
«Изменить») делает наивный `iso.slice(0, 16)` на UTC ISO-строке из API, БЕЗ конвертации в локальное
время браузера — в отличие от `formatDeadline(iso)` (колонка «Дедлайн» в таблице), который
корректно использует `toLocaleString(...)` с локальным часовым поясом. На машине с TZ
`Europe/Moscow` (UTC+3, подтверждено `Intl.DateTimeFormat().resolvedOptions().timeZone`)
воспроизведено так: (1) ADMIN вводит в форме редактирования `14:30` и сохраняет → колонка «Дедлайн»
корректно показывает `05.09.2026, 14:30`; (2) ADMIN снова открывает «Изменить» на этой же строке —
поле `datetime-local` показывает `2026-09-05T11:30` (UTC-значение как есть, без конвертации в
локальное время) — на 3 часа меньше того, что реально сохранено и показано в соседней колонке; (3)
если ADMIN нажимает «Сохранить» на этом экране, НЕ трогая поле дедлайна вообще (например, меняя
только `weekLabel`) — колонка «Дедлайн» после сохранения показывает уже `05.09.2026, 11:30` —
значение тихо сдвинулось на 3 часа назад без какого-либо явного действия администратора над этим
полем. Воспроизведено дважды (`TC-P8-UI-04` — прямая проверка значения инпута; отдельный скрипт
`p8_tz_drift.mjs` — сквозной сценарий save→reopen→save-без-изменений→сравнение отображаемого
значения до/после, лог: `After 1st save: 05.09.2026, 14:30` → `input shows: 2026-09-05T11:30` →
`After 2nd save: 05.09.2026, 11:30`, `DRIFT DETECTED: true`). **Исправление** — предмет основной
сессии, не мой мандат; вероятно, `toDateTimeInputValue` должен конвертировать через локальные
компоненты `Date` (`getFullYear`/`getMonth`/... или `date-fns`-эквивалент), а не резать строку.

**Дефект №2 (реальный, подтверждён) — уведомление `CYCLE_ARCHIVED` (и любое другое, ссылающееся на
цикл, который к моменту клика уже `ARCHIVED`) ведёт SPEAKER на функционально недостижимую страницу
`/presentation?cycle=<id>`, с противоречивым сообщением на экране.** `NotificationBell.targetUrl`
безусловно строит `/presentation?cycle=<weeklyCycleId>` для типов `CYCLE_ASSEMBLED`/
`CYCLE_ARCHIVED`, не различая, находится ли цикл всё ещё в `ASSEMBLED`, или уже успел перейти в
`ARCHIVED` к моменту клика. Но это прямо противоречит уже существующему (Этап 5а/6) ролевому
ограничению: `assertPresentationReadAccess` (`server/src/utils/presentationAccess.ts:14-16`) кидает
`403 "Презентация архивирована и недоступна"` для SPEAKER на `ARCHIVED`-циклах, И, отдельно,
`GET /api/weekly-cycles` для SPEAKER исключает `ARCHIVED`-циклы из списка вообще (`where: { status:
{ not: "ARCHIVED" } }`, `server/src/routes/weeklyCycles.ts:11`) — то есть на `AssemblePage` `<select
id="cycle">` для SPEAKER такой цикл никогда не появится среди `<option>`. Воспроизведено: спикер с
реальным `CYCLE_ARCHIVED`-уведомлением от нашего API-прогона (`weeklyCycleId` цикла, архивированного
в ходе теста) → прямой переход на `/presentation?cycle=<этот id>` → на экране ОДНОВРЕМЕННО: (а)
`<select id="cycle">` показывает пустой плейсхолдер «— выберите цикл —» (`inputValue() === ""`, не
совпадает с `weeklyCycleId` из URL, хотя `cycleId`-state в React корректно проинициализирован из
`?cycle=` — сам API-запрos `getPresentation(cycleId)` реально уходит с правильным id, что видно по
ошибке ниже), (б) `.error-text` = «Презентация архивирована и недоступна» (корректный 403 от
сервера), (в) СРАЗУ под ней ещё и `.hint-text` = «Презентация ещё не собрана.» — оба текста рендерятся
одновременно, потому что `view` остаётся `null` после сработавшего `.catch`, а условие показа этой
подсказки (`cycleId && !loading && !view?.presentation`) не учитывает, что `error` уже объясняет
недоступность по другой причине. Скриншот: `p8-deeplink-presentation.png` в scratch-каталоге сессии.
Контрольная проверка (`TC-P8-deep-2`): тот же переход на `ASSEMBLED`-но-ещё-НЕ-`ARCHIVED` цикл
работает корректно — `<select>` предвыбирает нужный цикл, презентация отображается — подтверждает,
что дефект специфичен именно для уже-архивных циклов, то есть именно для сценария, который
`CYCLE_ARCHIVED`-уведомление всегда описывает по построению (цикл, вызвавший уведомление, уже
`ARCHIVED` к моменту, когда уведомление вообще могло появиться в списке). Затрагивает только роль
SPEAKER (у ADMIN нет этого ограничения в `assertPresentationReadAccess`, но ADMIN и не получает
персистентных уведомлений в принципе — не владелец слайдов). **Исправление** — предмет основной
сессии; кандидаты: не давать `targetUrl` для `CYCLE_ARCHIVED` вести на `/presentation` для SPEAKER
(например, вести в никуда/на список циклов с явным текстом), либо смягчить/переформулировать доступ
для SPEAKER к архивным презентациям своих же уведомлений, либо хотя бы убрать дублирующий
`hint-text` при наличии `error`.

**Тестовые данные, оставленные в БД (не часть деливери):**
- Спикеры: `qa.speaker.p8.a.<ts>` (пароль `dc8cad506a91`), `qa.speaker.p8.b.<ts>` (пароль
  `5869f81accd3`), где `<ts>` = `1784737465384` (unix ms на момент создания в этой сессии).
- Циклы (все с префиксом `QA-P8-` + тот же/близкий `<ts>`): `QA-P8-<ts>` (главный сценарий —
  архивирован в ходе теста, содержит 3 слайда от 2 спикеров, презентацию с одним слотом),
  `QA-P8-overdue-<ts>`, `QA-P8-far-<ts>`, `QA-P8-revision-<ts>`, `QA-P8-empty-<ts>` (архивных не
  оставляет — presentation отсутствует, только заглушка), плюс из Playwright/доп. проверок:
  `QA-P8-UI-<ts2>`, `QA-P8-TZDRIFT-<ts2>`, `QA-P8-notarchived-<ts2>`, `QA-P8-legacy-<ts2>`
  (`<ts2>` — отдельные более поздние unix-таймстампы этой же сессии, точные значения — в выводе
  скриптов в scratch-каталоге, не сохранены отдельно в репозитории).
- Один легаси-шаблон (`layoutKind = null`, id `ff997538-0d7d-43f6-a175-2252649c30a6`) использован
  для регрессии, не изменён.
- Обнаружен (не создан в этой сессии) существующий mojibake-артефакт в БД: `Template.id =
  230542d2-0cac-4708-b605-c664d1c195d5` имеет `name`, состоящее из байтов U+FFFD (`efbfbd` в hex) —
  похоже на старое повреждение из более ранней сессии через `curl -d` с кириллицей (см.
  `[[feedback-curl-cyrillic-encoding]]`). Не связано с Этапом 8 — уведомления корректно показывают
  содержимое поля `name` как есть, каким бы оно ни было; сама некорректность данных — не дефект
  Этапа 8, оставлена как есть, не мой мандат её чистить.

**Оба дефекта исправлены основной сессией в этом же прогоне (2026-07-22) и повторно проверены.**
- Дефект №1: `toDateTimeInputValue` в `CyclesPage.tsx` переписан на конвертацию через локальные
  компоненты `Date` (`getFullYear`/`getMonth`/`getDate`/`getHours`/`getMinutes`, с `padStart`), вместо
  наивного `iso.slice(0, 16)`. Регресс подтверждён отдельным Playwright-скриптом: сохранение `14:30` →
  колонка показывает `14:30` → повторное открытие «Изменить» показывает тот же `14:30` (не `11:30`) →
  повторное сохранение без правки поля не сдвигает значение — дрейф не воспроизводится.
- Дефект №2: `NotificationBell.targetUrl` для `CYCLE_ARCHIVED` теперь возвращает `null` (тип сигнатуры
  расширен до `string | null`) — клик по такому уведомлению только отмечает его прочитанным и
  закрывает панель, без перехода на заведомо недоступный для SPEAKER `/presentation`. Дополнительно
  `AssemblePage.tsx` перестал показывать дублирующий `hint-text` «Презентация ещё не собрана» при уже
  выставленной `error` (условие получило `&& !error`). Регресс подтверждён отдельным Playwright-
  скриптом на реальном сценарии сборка→архивация→уведомление: клик по CYCLE_ARCHIVED не меняет URL,
  не показывает `.error-text`, уведомление помечается прочитанным.
- `tsc --noEmit` чист в `client/` после обеих правок. Полный API-набор (44 проверки) и исходный
  Playwright-набор (6 проверок) основной сессии перепрогнаны после фиксов — 0 регрессий.

**Не сделано / не проверялось:**
- Не проверялась одновременная работа `Notification`-очереди при по-настоящему высокой конкурентности
  (>5 параллельных запросов, разные пользователи одновременно) — только 5 параллельных на один id
  одним пользователем.
- Не проверялась миграция `20260721200930_add_notifications` на пустой/чистой БД «с нуля» (только
  на уже мигрированной dev-БД).

### 2026-07-21 — Независимая проверка Этапа 7 (реальная загрузка/хранение `chart-image`) — дефектов не найдено

**Что проверялось:** отдельный от основной сессии реализации проход, нацеленный конкретно на
7 пробелов, явно перечисленных заказчиком проверки как НЕ покрытые собственным тестированием
основной сессии (её собственный прогон — 30 API-утверждений + Playwright-обход, см. её описание
задачи; сюда не дублируется). Реализация — план `C:\Users\mille\.claude\plans\
scalable-meandering-summit.md`, требования — `REQUIREMENTS.md`, секция «Этап 7» (черновик с
открытыми вопросами; ответы зафиксированы только в плане, не отдельной записью в
`REQUIREMENTS.md` — см. «Не сделано» ниже). Дев-сервер/клиент/Postgres уже были подняты
(`127.0.0.1:4000`/`127.0.0.1:5173`), последняя миграция `20260721170741_add_chart_image_asset`
уже применена.

**Инструменты:** Node 24 (встроенный `fetch`/`FormData`) для API-скрипта, `npx playwright@1.61.1`
(браузер уже закэширован в `~/AppData/Local/ms-playwright`, `chromium-1228`) для UI-скриптов —
оба временно запускались из `server/` (для резолва пакета `playwright` из
`server/node_modules`), сами скрипты и артефакты (скриншоты, тестовые PNG/JPEG-фикстуры) сохранены
в scratch-каталоге сессии, не в репозитории.

**1. Directory traversal / мусорные `assetId` на `GET /api/uploads/chart-images/:assetId`
(TC-P7-01/02/03).** 10 вариантов мусорных значений (path traversal тремя способами — сырой,
URL-кодированный, обратные слэши Windows-стиля; SQL-инъекция как строка; не-UUID; nil-UUID; `%00`;
строка на 5000 символов; `.`/`..`) плюс отдельная проверка двойного URL-кодирования `..%2f` на
HTTP-уровне (в обход клиентского `encodeURIComponent`, имитирует прямой недоверенный запрос) — все
10+1 дали чистый `404 {"error": "Файл не найден"}`, ни разу не 500, `GET /api/health` мгновенно
ответил сразу после серии — сервер не пострадал. Причина устойчивости подтверждена по коду
(`server/src/routes/uploads.ts:12-17`): `assetId` используется только как Prisma PK в
`findUnique`, не участвует в построении файлового пути до того, как запись реально найдена в БД —
`path.join(CHART_IMAGE_DIR, asset.id)` вызывается только с уже проверенным через БД UUID, не с
сырым вводом клиента. Content-Type раздачи подтверждён корректным для обоих форматов (`image/png`,
`image/jpeg`) по словарю, не по клиентским данным.

**2. Множественные `CHART_IMAGE`-блоки на одном слайде — изоляция (TC-P7-08/09/10).** Шаблон с
двумя картиночными блоками; загрузка/замена/удаление на одном блоке ни разу не задела другой —
разные `asset.id`, оба независимо читаются, замена/удаление одного блока оставляет второй
полностью нетронутым (тот же `asset.id`, файл на месте), физически удаляется только
действительно замененный/удалённый файл.

**3. `templateBlockId`, не относящийся к этому слайду/блоку (TC-P7-04..07).** Блок из ДРУГОГО
шаблона, блок другого типа (`RICH_TEXT_SECTION`) на этом же слайде, и полностью произвольная
строка — во всех случаях `400`, ни разу не `500` и не тихая запись в чужой ресурс, ни для `POST`,
ни для `DELETE`.

**4. Взаимодействие общего `PATCH /api/slides/:id` с загрузкой картинки (TC-P7-11/12/13).**
Клик «Сохранить» сразу после загрузки без прочих изменений — `200` (no-op-гард срабатывает
верно). То же самое ОДНОВРЕМЕННО с реальным изменением другого (текстового) блока того же
слайда — `200`, текстовое изменение реально сохранилось, `path` картинки не тронут (проверено
повторным `GET`, не просто по ответу `PATCH`). Прямая попытка подставить произвольный `path`
через общий `PATCH` (в обход дедикейтед upload-роута) — `400`, и, что важнее, повторный `GET`
подтвердил отсутствие частичной порчи данных: реальный `path` остался прежним, инъецированная
строка нигде не сохранилась.

**5. Доступ на чтение картинки после реальной архивации цикла (TC-P7-14..18) — единственный
пункт, требовавший полного цикла submit → добавление в презентацию (цикл → `ASSEMBLED`) →
`POST /weekly-cycles/:id/archive`, который основная сессия не проходила в своём прогоне.**
До архивации не-владелец SPEAKER читал картинку `IN_PRESENTATION`-слайда (`200` — согласуется с
уже существующим правилом «просмотр презентации открыт всем для не-архивных циклов»). После
архивации тот же не-владелец получил `403` — доступ пересчитывается по актуальному статусу
цикла в момент запроса, не кэшируется с момента постановки в презентацию. Владелец слайда и
ADMIN сохранили доступ (`200`) после архивации, как и ожидалось. Мутация (upload) заблокирована
`403` для ОБЕИХ ролей после архивации — статусный гард в `checkImageMutationAccess` учитывает
`weeklyCycle.status !== "COLLECTING"` независимо от роли, ADMIN не получает исключения.

**6. `tsc --noEmit`** — чисто в обоих пакетах (`server/`, `client/`) после проверки, код оставлен
в исходном виде (правок не вносилось, как и требовалось).

**Дополнительно (вне явного списка пробелов, но задело сопутствующие изменения) —
Playwright, реальный UI:**
- Чекбокс «обязательное» для блока `CHART_IMAGE`, добавленного вручную (не через pptx-импорт), в
  конструкторе шаблонов (`/templates`) реально НЕ задизейблен и реально ставится в `checked` —
  снятие ограничения подтверждено кликом в браузере, не только чтением кода (скриншот
  `p7-01-template-required-chart-checkbox.png`). Шаблон с таким обязательным блоком сохраняется
  без ошибки.
- `isBlockEmpty` для `CHART_IMAGE` на `SlideFormPage` — полный цикл подсказки «— не заполнено»:
  видна для пустого обязательного блока (скриншот `p7-02-slide-empty-required-hint.png`) →
  пропадает сразу после загрузки без перезагрузки страницы (скриншот
  `p7-03-slide-uploaded-hint-gone.png`) → переживает `page.reload()` при повторном выборе
  цикла/шаблона (скриншот `p7-04b-slide-after-reload-persists.png`; чистый `reload()` без
  повторного выбора не тестировался как «баг» — селекторы цикла/шаблона в `SlideFormPage`
  осознанно локальный React-стейт, не URL/localStorage, сброс при простом `reload()` без
  повторного выбора ожидаем и не является дефектом этого этапа, не регрессия) → возвращается
  после «Удалить» (скриншот `p7-05-slide-after-delete-hint-back.png`).
- Клик «Сохранить» сразу после загрузки в реальном браузере (не только через прямой API-запрос) —
  без баннера ошибки, подпись «Сохранено» появляется (скриншот
  `p7-04-slide-save-after-upload-no-error.png`).
- ADMIN на `/review` реально видит `ChartImageEditor` (кликабельный `<input type="file">`, не
  текст-заглушку) для `CHART_IMAGE`-блока чужого `DRAFT`-слайда, загружает файл через браузер —
  превью появляется на экране ADMIN, а повторный `GET /slides` от имени владельца-SPEAKER (через
  API) подтверждает тот же `assetId` (скриншот `p7-07-review-admin-after-upload.png`) — реальный
  сквозной путь для открытого вопроса №1 из `REQUIREMENTS.md` («ADMIN может редактировать чужой
  слайд»), не только API-уровневая проверка.
- Побочно замечено (не дефект, зафиксировано для будущих прогонов): тестовая фикстура-картинка
  1×1 пиксель, использованная для скорости, реально рендерится в браузере как физически заметный
  1-пиксельный контур из-за `max-width`/`max-height` без `min-*` у `.chart-image-preview`
  (`client/src/styles.css:460`) — на скриншотах видна как одиночная точка вместо заметного
  превью. Не баг: класс правильно ограничивает МАКСИМАЛЬНЫЙ размер, просто не задаёт визуально
  приятный минимум для вырожденно маленьких файлов — с реальными картинками графиков (десятки—
  сотни КБ, реальное разрешение) это не проявляется. Отмечено на будущее как возможное косметическое
  улучшение (не относится к объёму Этапа 7).

**Целостность файлов на диске (TC-P7-30).** После всех upload/replace/delete-циклов этого
прогона (плюс всё, что оставила основная сессия своим собственным прогоном ранее в тот же день)
количество файлов в `server/uploads/chart-images/` (22) точно совпало с количеством строк
`ChartImageAsset` в БД (22) — ни одного файла-сироты, ни одной записи без файла на диске.

**Найденные дефекты: НИ ОДНОГО.** Все 91 проверка (56 API-утверждений в первом скрипте + 25
Playwright-утверждений в двух UI-скриптах + 2 прогона `tsc --noEmit`; из них покрывают именно 7
явно запрошенных пробелов TC-P7-01..30 плюс несколько прилегающих e2e-проверок TC-P7-21..29)
прошли с первого раза. Ни одного нового вопроса к плану/требованиям не возникло сверх уже
зафиксированных в `REQUIREMENTS.md` открытых вопросов (все были явно закрыты решениями,
перечисленными в плане `scalable-meandering-summit.md`, раздел «Контекст» — «Ключевые решения»).

**Тестовые данные, оставленные в БД (не удалялись, по конвенции проекта):**
- Спикеры: `qa.speaker.p7.<ts>`, `qa.speaker.p7b.<ts>` (API-скрипт), `qa.speaker.p7ui.<ts>`
  (Playwright-скрипт 1), `qa.speaker.p7review.<ts>` (Playwright-скрипт 2) — 4 конкретных `<ts>`
  из этого прогона, см. вывод скриптов для точных логинов (метка времени `Date.now()` на момент
  прогона, 2026-07-21).
- Циклы: `2026-QA-P7-independent-<ts>` (заархивирован в ходе TC-P7-14..18 — намеренно, для
  проверки), `2026-QA-P7-independent-cycle2-<ts>` (остался `COLLECTING`), `2026-QA-P7-UI-<ts>`
  (остался `COLLECTING`), `2026-QA-P7-review-<ts>` (остался `COLLECTING`).
- Шаблоны: `QA-P7 Two Charts <ts>` (2 картиночных блока), `QA-P7 Foreign Template <ts>`
  (вспомогательный, для проверки чужого `templateBlockId`), `QA-P7 Required Chart <ts>`
  (обязательный картиночный блок, добавлен через UI-конструктор), `QA-P7 Review Chart <ts>`.
- На диске (`server/uploads/chart-images/`) — 22 файла, все с соответствующими строками
  `ChartImageAsset` в БД, включая финальные «выжившие» ассеты созданных выше слайдов.

**Не сделано / вне объёма этого прогона:**
- `REQUIREMENTS.md` для Этапа 7 по-прежнему содержит только черновик с открытыми вопросами
  (запись от 2026-07-21, «реализация ещё не начата») — фактические принятые решения видны только
  в плане (`scalable-meandering-summit.md`, раздел «Контекст»), не оформлены отдельной
  «Ответы на открытые вопросы»-записью в `REQUIREMENTS.md` по образцу предыдущих этапов (см.
  напр. «2026-07-19 — Этап 5: ответы на открытые вопросы аналитика»). Не дефект тестирования,
  но стоит на заметку для документации.
- Глубокая повторная проверка magic-byte sniffing/лимита 5 МБ/переименованного не-PNG-файла не
  делалась — по словам заказчика проверки уже покрыта собственным прогоном основной сессии,
  решил не дублировать.
- PDF-экспорт с реальной картинкой (фикс гонки `networkidle` в `browser.ts`) не перепроверялся
  отдельно — тоже уже подтверждён основной сессией («в PDF реально виден график»), вне списка
  явно запрошенных пробелов этого прогона.
